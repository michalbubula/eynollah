...
class EynollahPlotter():
    ...
    def save_deskewed_image(self, slope_deskew):
        if self.dirs.dir_of_all is not None:
            cv2.imwrite(os.path.join(self.dirs.dir_of_all, self.image_filename_stem + "_org.png"), self.image_org)
        if self.dirs.dir_of_deskewed is not None:
            img_rotated = rotate_image_different(self.image_org, slope_deskew)
            cv2.imwrite(os.path.join(self.dirs.dir_of_deskewed, self.image_filename_stem + "_deskewed.png"), img_rotated)

    def save_page_image(self, image_page):
        if self.dirs.dir_of_all is not None:
            cv2.imwrite(os.path.join(self.dirs.dir_of_all, self.image_filename_stem + "_page.png"), image_page)
        if self.dirs.dir_save_page is not None:
            cv2.imwrite(os.path.join(self.dirs.dir_save_page, self.image_filename_stem + "_page.png"), image_page)
    def save_enhanced_image(self, img_res):
        cv2.imwrite(os.path.join(self.dirs.dir_out, self.image_filename_stem + "_enhanced.png"), img_res)
    ...
    def save_plot_of_rotation_angle(self, angels, var_res):
        if self.dirs.dir_of_all is not None:
            ...
            plt.legend(loc='best')
            plt.savefig(os.path.join(self.dirs.dir_of_all, self.image_filename_stem+'_rotation_angle.png'))

    def write_images_into_directory(self, img_contours, image_page):
        if self.dirs.dir_of_cropped_images is not None:
            index = 0
