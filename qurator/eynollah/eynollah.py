...
class Eynollah:
    def __init__(
        ...
        self.model_dir_of_col_classifier = (
            dir_models + "/eynollah-column-classifier_20210425"
        )
        self.model_region_dir_p = (
            dir_models + "/eynollah-main-regions-aug-scaling_20210425"
        )
        self.model_region_dir_p2 = (
            dir_models + "/eynollah-main-regions-aug-rotation_20210425"
        )
        self.model_region_dir_fully_np = (
            dir_models + "/eynollah-full-regions-1column_20210425"
        )
        self.model_region_dir_fully = (
            dir_models + "/eynollah-full-regions-3+column_20210425"
        )
        self.model_page_dir = dir_models + "/eynollah-page-extraction_20210425"
        self.model_region_dir_p_ens = (
            dir_models + "/eynollah-main-regions-ensembled_20210425"
        )
        self.model_region_dir_p_ens_light = (
            dir_models + "/eynollah-main-regions_20220314"
        )
        ...
        if dir_in and light_version:
            ...
            self.model_classifier = self.our_load_model(
                self.model_dir_of_col_classifier
            )
            ...
            self.model_region_fl_np = self.our_load_model(
                self.model_region_dir_fully_np
            )
    ...
    def predict_enhancement(self, img):
        ...
        model_enhancement, session_enhancement = self.start_new_session_and_model(
            self.model_dir_of_enhancement
        )

        img_height_model = model_enhancement.layers[
            len(model_enhancement.layers) - 1
        ].output_shape[1]
        img_width_model = model_enhancement.layers[
            len(model_enhancement.layers) - 1
        ].output_shape[2]
        if img.shape[0] < img_height_model:
            img = cv2.resize(
                img, (img.shape[1], img_width_model), interpolation=cv2.INTER_NEAREST
            )

        if img.shape[1] < img_width_model:
            img = cv2.resize(
                img, (img_height_model, img.shape[0]), interpolation=cv2.INTER_NEAREST
            )
        ...
        for i in range(nxf):
            for j in range(nyf):
                ...
                label_p_pred = model_enhancement.predict(
                    img_patch.reshape(
                        1, img_patch.shape[0], img_patch.shape[1], img_patch.shape[2]
                    ),
                    verbose=0,
                )
                ...
                if i == 0 and j == 0:
                    ...
                    seg = seg[
                        margin: seg.shape[0] - margin, margin: seg.shape[1] - margin
                    ]
                    prediction_true[
                        index_y_d + margin: index_y_u - margin,
                        index_x_d + margin: index_x_u - margin,
    ...
    def calculate_width_height_by_columns(
        self, img, num_col, width_early, label_p_pred
    ):
    ...
    def resize_image_with_column_classifier(self, is_image_enhanced, img_bin):
        ...
        if not self.dir_in:
            model_num_classifier, session_col_classifier = (
                self.start_new_session_and_model(self.model_dir_of_col_classifier)
            )
        if self.input_binary:
            img_in = np.copy(img)
            img_in = img_in / 255.0
            ...
            img_1ch = img_1ch[
                page_coord[0]: page_coord[1], page_coord[2]: page_coord[3]
            ]
        ...
        img_new, _ = self.calculate_width_height_by_columns(
    ...
    def resize_and_enhance_image_with_column_classifier(self, light_version):
        ...
        if self.input_binary:
            ...
            if self.dir_in:
                ...
                model_bin, session_bin = self.start_new_session_and_model(
                    self.model_dir_of_binarization
                )
        ...
        self.logger.info(
            "Found %d columns (%s)", num_col, np.around(label_p_pred, decimals=5)
        )
    ...
    def do_prediction_new_concept(
        self, patches, img, model, marginal_of_patch_percent=0.1
    ):
        self.logger.debug("enter do_prediction")
            ...
            for i in range(nxf):
                for j in range(nyf):
                    ...
                    if i == 0 and j == 0:
                        ...
                        seg = seg[
                            margin: seg.shape[0] - margin, 0: seg.shape[1] - margin
                        ]
                        mask_true[
                            index_y_d + margin: index_y_u - margin,
                            index_x_d + 0: index_x_u - margin,
                        ...
                        seg = seg[
                            margin: seg.shape[0] - margin, margin: seg.shape[1] - 0
                        ]
                        mask_true[
                            index_y_d + margin: index_y_u - margin,
                            index_x_d + margin: index_x_u - 0,
                        ...
                        seg = seg[
                            0: seg.shape[0] - margin, margin: seg.shape[1] - margin
                        ]
                        mask_true[
                            index_y_d + 0: index_y_u - margin,
                            index_x_d + margin: index_x_u - margin,
                        ...
                        seg = seg[
                            margin: seg.shape[0] - 0, margin: seg.shape[1] - margin
                        ]
                        mask_true[
                            index_y_d + margin: index_y_u - 0,
                            index_x_d + margin: index_x_u - margin,
                        ...
                        seg = seg[
                            margin: seg.shape[0] - margin,
                            margin: seg.shape[1] - margin,
                        ]
                        mask_true[
                            index_y_d + margin: index_y_u - margin,
                            index_x_d + margin: index_x_u - margin,
    ...
    def extract_text_regions(self, img, patches, cols):
        ...
        if not self.dir_in:
            model_region, session_region = self.start_new_session_and_model(
                self.model_region_dir_fully
                if patches
                else self.model_region_dir_fully_np
            )
        else:
        ...
        if not patches:
            ...
            if cols == 1:
                ...
                img2 = resize_image(
                    img2, int(img_height_h * 0.7), int(img_width_h * 0.7)
                )
                marginal_of_patch_percent = 0.1
                prediction_regions2 = self.do_prediction(
                    patches, img2, model_region, marginal_of_patch_percent
                )
                prediction_regions2 = resize_image(
                    prediction_regions2, img_height_h, img_width_h
                )
            ...
            if cols == 2:
                ...
                img2 = img2.astype(np.uint8)
                img2 = resize_image(
                    img2, int(img_height_h * 0.4), int(img_width_h * 0.4)
                )
                marginal_of_patch_percent = 0.1
                prediction_regions2 = self.do_prediction(
                    patches, img2, model_region, marginal_of_patch_percent
                ...
                img2 = img2.astype(np.uint8)
                img2 = resize_image(
                    img2, int(img_height_h * 0.3), int(img_width_h * 0.3)
                )
                marginal_of_patch_percent = 0.1
                prediction_regions2 = self.do_prediction(
                    patches, img2, model_region, marginal_of_patch_percent
            ...
            if cols == 2:
                ...
                if img_width_h >= 2000:
                    img = resize_image(
                        img, int(img_height_h * 0.9), int(img_width_h * 0.9)
                    )
                img = img.astype(np.uint8)
            ...
            if cols == 3:
                if (self.scale_x == 1 and img_width_h > 3000) or (
                    self.scale_x != 1 and img_width_h > 2800
                ):
                    img = otsu_copy_binary(img)
                    img = img.astype(np.uint8)
                    img = resize_image(
                        img, int(img_height_h * 2800 / float(img_width_h)), 2800
                    )
                else:
                    img = otsu_copy_binary(img)
                    img = img.astype(np.uint8)
            ...
            if cols == 4:
                if (self.scale_x == 1 and img_width_h > 4000) or (
                    self.scale_x != 1 and img_width_h > 3700
                ):
                    img = otsu_copy_binary(img)
                    img = img.astype(np.uint8)
                    img = resize_image(
                        img, int(img_height_h * 3700 / float(img_width_h)), 3700
                    )
                else:
                    img = otsu_copy_binary(img)
                    img = img.astype(np.uint8)
            ...
            if cols == 5:
                if self.scale_x == 1 and img_width_h > 5000:
                    ...
                    img = img.astype(np.uint8)
                    img = resize_image(
                        img, int(img_height_h * 0.7), int(img_width_h * 0.7)
                    )
                else:
                    img = otsu_copy_binary(img)
                    img = img.astype(np.uint8)
            ...
            if cols >= 6:
                if img_width_h > 5600:
                    ...
                    img = img.astype(np.uint8)
                    img = resize_image(
                        img, int(img_height_h * 5600 / float(img_width_h)), 5600
                    )
                else:
                    img = otsu_copy_binary(img)
                    img = img.astype(np.uint8)
        ...
        prediction_regions = self.do_prediction(
            patches, img, model_region, marginal_of_patch_percent
        )
        prediction_regions = resize_image(prediction_regions, img_height_h, img_width_h)
        self.logger.debug("exit extract_text_regions")
        return prediction_regions, prediction_regions2

    def get_slopes_and_deskew_new_light(
        self,
        contours,
        contours_par,
        textline_mask_tot,
        ...
        for i in range(num_cores):
            ...
            indexes_text_con_per_process = indexes_by_text_con[
                int(nh[i]): int(nh[i + 1])
            ]

            processes.append(
                Process(
                    target=self.do_work_of_slopes_new_light,
                    args=(
                        queue_of_all_params,
                        boxes_per_process,
                        textline_mask_tot,
                        contours_per_process,
                        contours_par_per_process,
                        indexes_text_con_per_process,
                        image_page_rotated,
                        slope_deskew,
                    ),
                )
    ...
    def get_slopes_and_deskew_new(
        ...
        for i in range(num_cores):
            ...
            processes.append(
                Process(
                    target=self.do_work_of_slopes_new,
                    args=(
                        queue_of_all_params,
                        boxes_per_process,
                        textline_mask_tot,
    ...
    def get_slopes_and_deskew_new_curved(
        ...
        for i in range(num_cores):
            ...
            processes.append(
                Process(
                    target=self.do_work_of_slopes_new_curved,
                    args=(
                        queue_of_all_params,
                        boxes_per_process,
                        textline_mask_tot,
        ...
        for mv in range(len(boxes_text)):
            ...
            if img_int_p.shape[0] / img_int_p.shape[1] < 0.1:
                ...
                try:
                    ...
                    textline_con_fil = filter_contours_area_of_image(
                        img_int_p, textline_con, hierarchy, max_area=1, min_area=0.0008
                    )
                    ...
                    if self.isNaN(y_diff_mean):
                        ...
                        slope_for_all = return_deskew_slop(
                            img_int_p, sigma_des, plotter=self.plotter
                        )
            ...
            if abs(slope_for_all) < 45:
                ...
                mask_biggest = cv2.fillPoly(
                    mask_biggest, pts=[cnt_o_t_max], color=(1, 1, 1)
                )
                ...
                textline_rotated_separated = separate_lines_new2(
                    textline_biggest_region[y: y + h, x: x + w],
                    0,
                    num_col,
                    slope_for_all,
                ...
                textline_cnt_separated[y: y + h, x: x + w] = (
                    textline_rotated_separated
                )
                textline_region_in_image[y: y + h, x: x + w] = (
                    textline_rotated_separated
                )
                ...
                pixel_img = 1
                cnt_textlines_in_image = return_contours_of_interested_textline(
                    textline_region_in_image, pixel_img
                )

                textlines_cnt_per_region = []
                for jjjj in range(len(cnt_textlines_in_image)):
                    mask_biggest2 = np.zeros(mask_texts_only.shape)
                    mask_biggest2 = cv2.fillPoly(
                        mask_biggest2,
                        pts=[cnt_textlines_in_image[jjjj]],
                        color=(1, 1, 1),
                    )
                    if num_col + 1 == 1:
                    ...
                    pixel_img = 1
                    mask_biggest2 = resize_image(
                        mask_biggest2,
                        int(mask_biggest2.shape[0] * scale_par),
                        int(mask_biggest2.shape[1] * scale_par),
                    )
                    cnt_textlines_in_image_ind = return_contours_of_interested_textline(
                        mask_biggest2, pixel_img
                    )
                    try:
                ...
                textlines_cnt_per_region = textline_contours_postprocessing(
                    all_text_region_raw,
                    slope_for_all,
            ...
            contours_textregion_par_per_each_subprocess.append(
                contours_par_per_process[mv]
            )
    ...
    def do_work_of_slopes_new_light(
        ...
        for mv in range(len(boxes_text)):
            ...
            mask_textline = cv2.fillPoly(
                mask_textline, pts=[contours_per_process[mv]], color=(1, 1, 1)
            )
            ...
            mask_only_con_region = cv2.fillPoly(
                mask_only_con_region,
                pts=[contours_par_per_process[mv]],
                color=(1, 1, 1),
            )
            ...
            if self.textline_light:
                ...
                cnt_clean_rot_raw, hir_on_cnt_clean_rot = return_contours_of_image(
                    all_text_region_raw
                )
                cnt_clean_rot = filter_contours_area_of_image(
                    all_text_region_raw,
                    cnt_clean_rot_raw,
                    hir_on_cnt_clean_rot,
                    max_area=1,
                    min_area=0.00001,
                )
            else:
                ...
                mask_only_con_region = mask_only_con_region[
                    boxes_text[mv][1]: boxes_text[mv][1] + boxes_text[mv][3],
                    boxes_text[mv][0]: boxes_text[mv][0] + boxes_text[mv][2],
                ]
                all_text_region_raw[mask_only_con_region == 0] = 0
        ...
        for mv in range(len(boxes_text)):
            ...
            if img_int_p.shape[0] / img_int_p.shape[1] < 0.1:
                ...
                try:
                    textline_con, hierarchy = return_contours_of_image(img_int_p)
                    textline_con_fil = filter_contours_area_of_image(
                        img_int_p, textline_con, hierarchy, max_area=1, min_area=0.00008
                    )
                    y_diff_mean = find_contours_mean_y_diff(textline_con_fil)
                    if self.isNaN(y_diff_mean):
                        slope_for_all = MAX_SLOPE
    ...
    def textline_contours(self, img, patches, scaler_h, scaler_w):
        ...
        img = resize_image(
            img_org, int(img_org.shape[0] * scaler_h), int(img_org.shape[1] * scaler_w)
        )
        if not self.dir_in:
        ...
        if not self.dir_in:
            prediction_textline_longshot = self.do_prediction(
                False, img, model_textline
            )
        else:
            prediction_textline_longshot = self.do_prediction(
                False, img, self.model_textline
            )
        prediction_textline_longshot_true_size = resize_image(
            prediction_textline_longshot, img_h, img_w
        )

        if self.textline_light:
            return (prediction_textline[:, :, 0] == 1) * 1, (
                prediction_textline_longshot_true_size[:, :, 0] == 1
            ) * 1
        else:
            return (
                prediction_textline[:, :, 0],
                prediction_textline_longshot_true_size[:, :, 0],
            )

    def do_work_of_slopes(
        self,
        q,
        poly,
        box_sub,
        ...
        for mv in range(len(boxes_per_process)):
            crop_img, _ = crop_image_inside_box(
                boxes_per_process[mv],
            ...
            try:
                ...
                textline_con_fil = filter_contours_area_of_image(
                    crop_img, textline_con, hierarchy, max_area=1, min_area=0.0008
                )
                y_diff_mean = find_contours_mean_y_diff(textline_con_fil)
                sigma_des = max(1, int(y_diff_mean * (4.0 / 40.0)))
                crop_img[crop_img > 0] = 1
                slope_corresponding_textregion = return_deskew_slop(
                    crop_img, sigma_des, plotter=self.plotter
                )
            except Exception as why:
                self.logger.error(why)
    ...
    def get_regions_light_v(self, img, is_image_enhanced, num_col_classifier):
        ...
        if not self.dir_in:
            ...
            prediction_regions_org = self.do_prediction_new_concept(
                True, img_bin, model_region
            )
        else:
            prediction_regions_org = self.do_prediction_new_concept(
                True, img_bin, self.model_region
            )

        # plt.imshow(prediction_regions_org[:,:,0])
        # plt.show()

        prediction_regions_org = resize_image(
            prediction_regions_org, img_height_h, img_width_h
        )
        textline_mask_tot_ea = resize_image(
            textline_mask_tot_ea, img_height_h, img_width_h
        )
        ...
        polygons_of_only_texts = return_contours_of_interested_region(
            mask_texts_only, 1, 0.00001
        )

        polygons_of_only_lines = return_contours_of_interested_region(
            mask_lines_only, 1, 0.00001
        )

        text_regions_p_true = np.zeros(prediction_regions_org.shape)

        text_regions_p_true = cv2.fillPoly(
            text_regions_p_true, pts=polygons_of_only_lines, color=(3, 3, 3)
        )

        text_regions_p_true[:, :][mask_images_only[:, :] == 1] = 2

        text_regions_p_true = cv2.fillPoly(
            text_regions_p_true, pts=polygons_of_only_texts, color=(1, 1, 1)
        )
    ...
    def get_regions_from_xy_2models(self, img, is_image_enhanced, num_col_classifier):
        ...
        img = resize_image(
            img_org, int(img_org.shape[0] * ratio_y), int(img_org.shape[1] * ratio_x)
        )
        if not self.dir_in:
        ...
        prediction_regions_org_y = resize_image(
            prediction_regions_org_y, img_height_h, img_width_h
        )
        ...
        try:
            img_only_regions = cv2.erode(
                img_only_regions_with_sep[:, :], KERNEL, iterations=20
            )

            _, _ = find_num_col(
                img_only_regions, num_col_classifier, self.tables, multiplier=6.0
            )
            ...
            if self.dir_in:
                prediction_regions_org = self.do_prediction(
                    True, img, self.model_region
                )
            else:
            ...
            prediction_regions_org[
                (prediction_regions_org[:, :] == 1) & (mask_zeros_y[:, :] == 1)
            ] = 0
            ...
            if self.dir_in:
                prediction_regions_org2 = self.do_prediction(
                    True, img, self.model_region_p2, 0.2
                )
            else:
                prediction_regions_org2 = self.do_prediction(
                    True, img, model_region, 0.2
                )
            prediction_regions_org2 = resize_image(
                prediction_regions_org2, img_height_h, img_width_h
            )
            ...
            prediction_regions_org_copy[
                (prediction_regions_org_copy[:, :] == 1) & (mask_zeros2[:, :] == 1)
            ] = 0
            ...
            if not (
                is_image_enhanced and rate_two_models < RATIO_OF_TWO_MODEL_THRESHOLD
            ):
                prediction_regions_org = np.copy(prediction_regions_org_copy)

            prediction_regions_org[
                (mask_lines2[:, :] == 1) & (prediction_regions_org[:, :] == 0)
            ] = 3
            mask_lines_only = (prediction_regions_org[:, :] == 3) * 1
            prediction_regions_org = cv2.erode(
                prediction_regions_org[:, :], KERNEL, iterations=2
            )

            prediction_regions_org = cv2.dilate(
                prediction_regions_org[:, :], KERNEL, iterations=2
            )
            ...
            if rate_two_models <= 40:
                if self.input_binary:
                    ...
                    if not self.dir_in:
                        ...
                        prediction_bin = self.do_prediction(
                            True, img_org, self.model_bin
                        )
                    prediction_bin = resize_image(
                        prediction_bin, img_height_h, img_width_h
                    )
                    ...
                    prediction_bin = np.repeat(
                        prediction_bin[:, :, np.newaxis], 3, axis=2
                    )
            ...
            polygons_lines_xml, hir_lines_xml = return_contours_of_image(
                mask_lines_only
            )
            polygons_lines_xml = textline_con_fil = filter_contours_area_of_image(
                mask_lines_only,
    ...
    def do_order_of_regions_full_layout(
        ...
        cx_text_only, cy_text_only, x_min_text_only, _, _, _, y_cor_x_min_main = (
        ...
        (
            ...
            _,
            y_cor_x_min_main_h,
        ) = find_new_features_of_contours(contours_only_text_parent_h)
        ...
        try:
            ...
            for iij in range(len(boxes)):
                ...
                (
                    ...
                    index_by_kind_sorted,
                ) = order_of_regions(
                    textline_mask_tot[
                        int(boxes[iij][2]): int(boxes[iij][3]),
                        int(boxes[iij][0]): int(boxes[iij][1]),
                    ],
                    con_inter_box,
                    con_inter_box_h,
                    boxes[iij][2],
                ...
                indexes_sorted_main = np.array(indexes_sorted)[
                    np.array(kind_of_texts_sorted) == 1
                ]
                indexes_by_type_main = np.array(index_by_kind_sorted)[
                    np.array(kind_of_texts_sorted) == 1
                ]
                indexes_sorted_head = np.array(indexes_sorted)[
                    np.array(kind_of_texts_sorted) == 2
                ]
                indexes_by_type_head = np.array(index_by_kind_sorted)[
                    np.array(kind_of_texts_sorted) == 2
                ]
                ...
                for zahler, _ in enumerate(args_contours_box):
                    ...
                    order_by_con_main[
                        args_contours_box[indexes_by_type_main[zahler]]
                ...
                for zahler, _ in enumerate(args_contours_box_h):
                    ...
                    order_by_con_head[
                        args_contours_box_h[indexes_by_type_head[zahler]]
                    ] = (np.where(indexes_sorted == arg_order_v)[0][0] + ref_point)
            ...
            for iii in range(len(order_of_texts_tot)):
                order_text_new.append(
                    np.where(np.array(order_of_texts_tot) == iii)[0][0]
                )
        ...
        try:
            ...
            for iij in range(len(boxes)):
                ...
                for i in range(len(args_contours_box)):
                    con_inter_box.append(
                        contours_only_text_parent[args_contours_box[i]]
                    )
    ...
    def check_iou_of_bounding_box_and_contour_for_tables(
        self, layout, table_prediction_early, pixel_tabel, num_col_classifier
    ):
        ...
        for i in range(len(contours)):
            ...
            if iou < 80:
                ...
                layout_contour = cv2.fillPoly(
                    layout_contour, pts=[contours[i]], color=(1, 1, 1)
                )
                ...
                layout_contour_sum_diff_smoothed = gaussian_filter1d(
                    layout_contour_sum_diff, 10
                )
                ...
                contours_sep, _ = cv2.findContours(
                    thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
                )
                ...
                for ji in range(len(contours_sep)):
                    ...
                    if num_col_classifier >= 2:
                        only_recent_contour_image = np.zeros(
                            (layout.shape[0], layout.shape[1])
                        )
                        only_recent_contour_image = cv2.fillPoly(
                            only_recent_contour_image,
                            pts=[contours_sep[ji]],
                            color=(1, 1, 1),
                        )
                        table_pixels_masked_from_early_pre = (
                            only_recent_contour_image[:, :]
                            * table_prediction_early[:, :]
                        )
                        iou_in = (
                            table_pixels_masked_from_early_pre.sum()
                            / float(only_recent_contour_image.sum())
                            * 100
                        )
                        # print(iou_in,'iou_in_in1')

                        if iou_in > 30:
                            layout_org = cv2.fillPoly(
                                layout_org,
                                pts=[contours_sep[ji]],
                ...
                if num_col_classifier >= 2:
                    ...
                    only_recent_contour_image = cv2.fillPoly(
                        only_recent_contour_image, pts=[contours[i]], color=(1, 1, 1)
                    )

                    table_pixels_masked_from_early_pre = (
                        only_recent_contour_image[:, :] * table_prediction_early[:, :]
                    )
                    iou_in = (
                        table_pixels_masked_from_early_pre.sum()
                        / float(only_recent_contour_image.sum())
    ...
    def delete_separator_around(
        self, spliter_y, peaks_neg, image_by_region, pixel_line, pixel_table
    ):
        ...
        if len(image_by_region.shape) == 3:
            for i in range(len(spliter_y) - 1):
                for j in range(1, len(peaks_neg[i]) - 1):
                    image_by_region[
                        int(spliter_y[i]): int(spliter_y[i + 1]),
    ...
    def add_tables_heuristic_to_layout(
        ...
        pixel_table = 10
        image_revised_1 = self.delete_separator_around(
            spliter_y, peaks_neg_tot, image_revised, pixel_line, pixel_table
        )

        try:
            image_revised_1[:, :30][image_revised_1[:, :30] == pixel_line] = 0
            image_revised_1[:, image_revised_1.shape[1] - 30:][
                image_revised_1[:, image_revised_1.shape[1] - 30:] == pixel_line
            ] = 0
        except:
            pass
        ...
        for indiv in np.unique(image_revised_1):
            ...
            if indiv == pixel_table:
                main_contours = filter_contours_area_of_image_tables(
                    thresh, contours, hirarchy, max_area=1, min_area=0.001
                )
            else:
                main_contours = filter_contours_area_of_image_tables(
                    thresh, contours, hirarchy, max_area=1, min_area=min_area
                )

            img_comm = cv2.fillPoly(
                img_comm, pts=main_contours, color=(indiv, indiv, indiv)
            )
            img_comm = img_comm.astype(np.uint8)

        if not self.isNaN(slope_mean_hor):
            image_revised_last = np.zeros(
                (image_regions_eraly_p.shape[0], image_regions_eraly_p.shape[1], 3)
            )
            for i in range(len(boxes)):
                ...
                try:
                    ...
                    contours_tab = filter_contours_area_of_image_tables(
                        image_box_tabels_1, contours_tab, _, 1, 0.003
                    )
                    image_box_tabels_1 = (image_box[:, :, 0] == pixel_line) * 1

                    image_box_tabels_and_m_text = (
                        (image_box[:, :, 0] == pixel_table) | (image_box[:, :, 0] == 1)
                    ) * 1
                    image_box_tabels_and_m_text = image_box_tabels_and_m_text.astype(
                        np.uint8
                    )

                    image_box_tabels_1 = image_box_tabels_1.astype(np.uint8)
                    image_box_tabels_1 = cv2.dilate(
                        image_box_tabels_1, KERNEL, iterations=5
                    )

                    contours_table_m_text, _ = return_contours_of_image(
                        image_box_tabels_and_m_text
                    )
                    image_box_tabels = np.repeat(
                        image_box_tabels_1[:, :, np.newaxis], 3, axis=2
                    )
                    ...
                    contours_line, hierachy = cv2.findContours(
                        thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
                    )

                    y_min_main_line, y_max_main_line = find_features_of_contours(
                        contours_line
                    )
                    y_min_main_tab, y_max_main_tab = find_features_of_contours(
                        contours_tab
                    )
                    ...
                    (
                        ...
                        y_max_tab_m_text,
                        _,
                    ) = find_new_features_of_contours(contours_table_m_text)
                    (
                        cx_tabl,
                        cy_tabl,
                        x_min_tabl,
                        ...
                        y_max_tabl,
                        _,
                    ) = find_new_features_of_contours(contours_tab)
                        ...
                        for i_t in range(len(y_min_main_tab)):
                            ...
                            for i_l in range(len(y_min_main_line)):
                                if (
                                    ...
                                    and y_max_main_tab[i_t] > y_min_main_line[i_l]
                                    and y_min_main_tab[i_t] > y_max_main_line[i_l]
                                    and y_max_main_tab[i_t] > y_min_main_line[i_l]
                                ):
                                    pass
                                elif (
                                    y_min_main_tab[i_t] < y_max_main_line[i_l]
                                    and y_max_main_tab[i_t] < y_max_main_line[i_l]
                                    and y_max_main_tab[i_t] < y_min_main_line[i_l]
                                    and y_min_main_tab[i_t] < y_min_main_line[i_l]
                                ):
                                    pass
                                elif (
                                    np.abs(y_max_main_line[i_l] - y_min_main_line[i_l])
                                    < 100
                                ):
                                    pass
                                else:
                                    y_up_tab.append(
                                        np.min(
                                            [y_min_main_line[i_l], y_min_main_tab[i_t]]
                                        )
                                    )
                                    y_down_tab.append(
                                        np.max(
                                            [y_max_main_line[i_l], y_max_main_tab[i_t]]
                                        )
                                    )
        ...
        if num_col_classifier == 1:
            ...
            _, _, _, _, y_min_tab_col1, y_max_tab_col1, _ = (
                find_new_features_of_contours(contours_table_col1)
            )

            if len(y_min_tab_col1) > 0:
                for ijv in range(len(y_min_tab_col1)):
                    image_revised_last[
                        int(y_min_tab_col1[ijv]): int(y_max_tab_col1[ijv]), :, :
                    ] = pixel_table
    ...
    def get_tables_from_model(self, img, num_col_classifier):
        ...
        if 4 > num_col_classifier > 2:
            ...
            pre_updown = self.do_prediction(
                patches, cv2.flip(img[:, :, :], -1), model_region
            )
            ...
            h_start = int(height_ext / 2.0)
            width_ext = int(img.shape[1] / 8.0)
            w_start = int(width_ext / 2.0)
            ...
            img_new[
                h_start: h_start + img.shape[0], w_start: w_start + img.shape[1], :
            ] = img[:, :, :]

            prediction_ext = self.do_prediction(patches, img_new, model_region)
            pre_updown = self.do_prediction(
                patches, cv2.flip(img_new[:, :, :], -1), model_region
            )
            pre_updown = cv2.flip(pre_updown, -1)

            prediction_table = prediction_ext[
                h_start: h_start + img.shape[0], w_start: w_start + img.shape[1], :
            ]
            prediction_table_updown = pre_updown[
                h_start: h_start + img.shape[0], w_start: w_start + img.shape[1], :
            ]
            ...
            img_w_half = int(img.shape[1] / 2.0)
            ...
            prediction_table_full_erode = cv2.erode(
                pre_full[:, :, 0], KERNEL, iterations=4
            )
            prediction_table_full_erode = cv2.dilate(
                prediction_table_full_erode, KERNEL, iterations=4
            )

            prediction_table_full_updown_erode = cv2.erode(
                pre_updown[:, :, 0], KERNEL, iterations=4
            )
            prediction_table_full_updown_erode = cv2.dilate(
                prediction_table_full_updown_erode, KERNEL, iterations=4
            )
        ...
        prediction_table_erode = cv2.erode(
            prediction_table[:, :, 0], KERNEL, iterations=20
        )
        prediction_table_erode = cv2.dilate(
            prediction_table_erode, KERNEL, iterations=20
        )
    ...
    def run_graphics_and_columns_light(
        ...
        if self.tables:
            table_prediction = self.get_tables_from_model(
                image_page, num_col_classifier
            )
        else:
            table_prediction = (
                np.zeros((image_page.shape[0], image_page.shape[1]))
            ).astype(np.int16)

        if self.plotter:
            self.plotter.save_page_image(image_page)

        text_regions_p_1 = text_regions_p_1[
            page_coord[0]: page_coord[1], page_coord[2]: page_coord[3]
        ]
        ...
        img_only_regions_with_sep = (
            (text_regions_p_1[:, :] != 3) & (text_regions_p_1[:, :] != 0)
        ) * 1
        img_only_regions_with_sep = img_only_regions_with_sep.astype(np.uint8)
        ...
        if erosion_hurts:
            ...
            img_only_regions = cv2.erode(
                img_only_regions_with_sep[:, :], KERNEL, iterations=6
            )
    ...
    def run_textline(self, image_page):
        ...
        textline_mask_tot_ea, _ = self.textline_contours(
            image_page, True, scaler_h_textline, scaler_w_textline
        )
        if self.textline_light:
    ...
    def run_deskew(self, textline_mask_tot_ea):
        ...
        slope_deskew = return_deskew_slop(
            cv2.erode(textline_mask_tot_ea, KERNEL, iterations=2),
            sigma,
            main_page_deskew,
            plotter=self.plotter,
        )
    ...
    def run_marginals(
        ...
        image_page_rotated, textline_mask_tot = (
            image_page[:, :],
            textline_mask_tot_ea[:, :],
        )
    ...
    def run_boxes_no_full_layout(
        ...
        if np.abs(slope_deskew) >= SLOPE_THRESHOLD:
            ...
            text_regions_p_1_n = resize_image(
                text_regions_p_1_n, text_regions_p.shape[0], text_regions_p.shape[1]
            )
            textline_mask_tot_d = resize_image(
                textline_mask_tot_d, text_regions_p.shape[0], text_regions_p.shape[1]
            )
            table_prediction_n = resize_image(
                table_prediction_n, text_regions_p.shape[0], text_regions_p.shape[1]
            )
        ...
        regions_without_separators = (
            text_regions_p[:, :] == 1
        ) * 1  # ( (text_regions_p[:,:]==1) | (text_regions_p[:,:]==2) )*1 #self.return_regions_without_separators_new(text_regions_p[:,:,0],img_only_regions)
        if self.tables:
            regions_without_separators[table_prediction == 1] = 1
        if np.abs(slope_deskew) < SLOPE_THRESHOLD:
        ...
        pixel_lines = 3
        if np.abs(slope_deskew) < SLOPE_THRESHOLD:
            _, _, matrix_of_lines_ch, splitter_y_new, _ = (
                find_number_of_columns_in_document(
        ...
        if np.abs(slope_deskew) >= SLOPE_THRESHOLD:
            _, _, matrix_of_lines_ch_d, splitter_y_new_d, _ = (
                find_number_of_columns_in_document(
        ...
        if num_col_classifier >= 3:
            if np.abs(slope_deskew) < SLOPE_THRESHOLD:
                regions_without_separators = regions_without_separators.astype(np.uint8)
                regions_without_separators = cv2.erode(
                    regions_without_separators[:, :], KERNEL, iterations=6
                )
            else:
                regions_without_separators_d = regions_without_separators_d.astype(
                    np.uint8
                )
                regions_without_separators_d = cv2.erode(
                    regions_without_separators_d[:, :], KERNEL, iterations=6
                )
        t1 = time.time()
        if np.abs(slope_deskew) < SLOPE_THRESHOLD:
            ...
            img_revised_tab2, contoures_tables = (
                self.check_iou_of_bounding_box_and_contour_for_tables(
                    img_revised_tab2, table_prediction, 10, num_col_classifier
                )
            )
        else:
            boxes_d, peaks_neg_tot_tables_d = (
                return_boxes_of_images_by_order_of_reading_new(
                    splitter_y_new_d,
            ...
            text_regions_p_tables[:, :][
                (text_regions_p_tables[:, :] != 3) & (table_prediction_n[:, :] == 1)
            ] = 10
            ...
            img_revised_tab2 = self.add_tables_heuristic_to_layout(
                ...
                0,
                splitter_y_new_d,
                peaks_neg_tot_tables_d,
                text_regions_p_tables,
                num_col_classifier,
                0.000005,
                pixel_line,
            )
            img_revised_tab2_d, _ = (
                self.check_iou_of_bounding_box_and_contour_for_tables(
            ...
            img_revised_tab2_d_rotated = resize_image(
                img_revised_tab2_d_rotated,
        ...
        if self.tables:
            if np.abs(slope_deskew) < SLOPE_THRESHOLD:
                ...
                img_revised_tab[:, :][
                    (text_regions_p[:, :] == 1) & (img_revised_tab[:, :] != 10)
                ] = 1
            else:
        ...
        polygons_of_marginals = return_contours_of_interested_region(
            text_regions_p, pixel_img, min_area_mar
        )

        pixel_img = 10
        contours_tables = return_contours_of_interested_region(
            text_regions_p, pixel_img, min_area_mar
        )
    ...
    def run_boxes_full_layout(
        ...
        if self.tables:
            ...
            regions_without_separators = (
                text_regions_p[:, :] == 1
            ) * 1  # ( (text_regions_p[:,:]==1) | (text_regions_p[:,:]==2) )*1 #self.return_regions_without_seperators_new(text_regions_p[:,:,0],img_only_regions)
            regions_without_separators[table_prediction == 1] = 1
            ...
            if num_col_classifier >= 3:
                if np.abs(slope_deskew) < SLOPE_THRESHOLD:
                    regions_without_separators = regions_without_separators.astype(
                        np.uint8
                    )
                    regions_without_separators = cv2.erode(
                        regions_without_separators[:, :], KERNEL, iterations=6
            ...
            if np.abs(slope_deskew) < SLOPE_THRESHOLD:
                ...
                img_revised_tab2_d_rotated = rotate_image(
                    img_revised_tab2_d, -slope_deskew
                )
        ...
        regions_fully, regions_fully_only_drop = self.extract_text_regions(
            image_page, True, cols=num_col_classifier
        )
        text_regions_p[:, :][regions_fully[:, :, 0] == 6] = 6
        regions_fully_only_drop = put_drop_out_from_only_drop_model(
            regions_fully_only_drop, text_regions_p
        )
        ...
        regions_fully_np, _ = self.extract_text_regions(
            image_page, False, cols=num_col_classifier
        )
        if num_col_classifier > 2:
            regions_fully_np[:, :, 0][regions_fully_np[:, :, 0] == 4] = 0
        else:
            regions_fully_np = filter_small_drop_capitals_from_no_patch_layout(
                regions_fully_np, text_regions_p
            )

        regions_fully = boosting_headers_by_longshot_region_segmentation(
            regions_fully, regions_fully_np, img_only_regions
        )
        ...
        # if not self.tables:
        if np.abs(slope_deskew) >= SLOPE_THRESHOLD:
            _, textline_mask_tot_d, text_regions_p_1_n, regions_fully_n = (
                rotation_not_90_func_full_layout(
                    image_page,
                    textline_mask_tot,
                    text_regions_p,
            ...
            regions_fully_n = resize_image(
                regions_fully_n, text_regions_p.shape[0], text_regions_p.shape[1]
            )
            if not self.tables:
                regions_without_separators_d = (text_regions_p_1_n[:, :] == 1) * 1
        else:
        ...
        for img_name in self.ls_imgs:
            ...
            if self.light_version:
                ...
                (
                    ...
                    table_prediction,
                    textline_mask_tot_ea,
                ) = self.run_graphics_and_columns_light(
                    text_regions_p_1,
                    textline_mask_tot_ea,
                    num_col_classifier,
                    num_column_is_classified,
                ...
                text_regions_p_1, erosion_hurts, polygons_lines_xml = (
                    self.get_regions_from_xy_2models(
                        img_res, is_image_enhanced, num_col_classifier
                    )
                )
            ...
            textline_mask_tot, text_regions_p, image_page_rotated = self.run_marginals(
                image_page,
                textline_mask_tot_ea,
                mask_images,
                mask_lines,
            ...
            if np.abs(slope_deskew) >= SLOPE_THRESHOLD:
                ...
                contours_only_text_parent = return_parent_contours(
                    contours_only_text, hir_on_text
                )

                if len(contours_only_text_parent) > 0:
                    areas_cnt_text = np.array(
                        [cv2.contourArea(c) for c in contours_only_text_parent]
                    )
                    areas_cnt_text = areas_cnt_text / float(
                        text_only.shape[0] * text_only.shape[1]
                    )
                    # self.logger.info('areas_cnt_text %s', areas_cnt_text)
                    contours_biggest = contours_only_text_parent[
                        np.argmax(areas_cnt_text)
                    ]
                    contours_only_text_parent = [
                        c
                        for jz, c in enumerate(contours_only_text_parent)
                        if areas_cnt_text[jz] > min_con_area
                    ]
                    areas_cnt_text_parent = [
                        area for area in areas_cnt_text if area > min_con_area
                    ]
                    index_con_parents = np.argsort(areas_cnt_text_parent)
                    contours_only_text_parent = list(
                        np.array(contours_only_text_parent, dtype=object)[
                            index_con_parents
                        ]
                    )
                    areas_cnt_text_parent = list(
                        np.array(areas_cnt_text_parent)[index_con_parents]
                    )

                    cx_bigest_big, cy_biggest_big, _, _, _, _, _ = (
                        find_new_features_of_contours([contours_biggest])
                    )
                    cx_bigest, cy_biggest, _, _, _, _, _ = (
                        find_new_features_of_contours(contours_only_text_parent)
                    )

                    contours_only_text_d, hir_on_text_d = return_contours_of_image(
                        text_only_d
                    )
                    contours_only_text_parent_d = return_parent_contours(
                        contours_only_text_d, hir_on_text_d
                    )

                    areas_cnt_text_d = np.array(
                        [cv2.contourArea(c) for c in contours_only_text_parent_d]
                    )
                    areas_cnt_text_d = areas_cnt_text_d / float(
                        text_only_d.shape[0] * text_only_d.shape[1]
                    )

                    if len(areas_cnt_text_d) > 0:
                        contours_biggest_d = contours_only_text_parent_d[
                            np.argmax(areas_cnt_text_d)
                        ]
                        index_con_parents_d = np.argsort(areas_cnt_text_d)
                        contours_only_text_parent_d = list(
                            np.array(contours_only_text_parent_d, dtype=object)[
                                index_con_parents_d
                            ]
                        )
                        areas_cnt_text_d = list(
                            np.array(areas_cnt_text_d)[index_con_parents_d]
                        )
                        ...
                        try:
                            if len(cx_bigest_d) >= 5:
                                ...
                                dists_d = [
                                    math.sqrt(
                                        (cx_bigest_big[0] - cx_bigest_d_last5[j]) ** 2
                                        + (cy_biggest_big[0] - cy_biggest_d_last5[j])
                                        ** 2
                                    )
            ...
            boxes_text, _ = get_text_region_boxes_by_given_contours(
                contours_only_text_parent
            )
            boxes_marginals, _ = get_text_region_boxes_by_given_contours(
                polygons_of_marginals
            )
            ...
            if not self.curved_line:
                if self.light_version:
                    ...
                    (
                        ...
                        all_box_coord,
                        index_by_text_par_con,
                    ) = self.get_slopes_and_deskew_new(
                        txt_con_org,
                        contours_only_text_parent,
                        textline_mask_tot_ea,
                        image_page_rotated,
                ...
                (
                    ...
                    txt_con_org,
                    contours_only_text_parent,
                    cv2.erode(textline_mask_tot_ea, kernel=KERNEL, iterations=1),
                    image_page_rotated,
                    boxes_text,
                    text_only,
                    num_col_classifier,
            ...
            if self.full_layout:
                if np.abs(slope_deskew) >= SLOPE_THRESHOLD:
                    contours_only_text_parent_d_ordered = list(
                        np.array(contours_only_text_parent_d_ordered, dtype=object)[
                            index_by_text_par_con
                        ]
                    )
                    if self.light_version:
                ...
                if not self.headers_off:
                    if np.abs(slope_deskew) < SLOPE_THRESHOLD:
                        num_col, _, matrix_of_lines_ch, splitter_y_new, _ = (
                            find_number_of_columns_in_document(
                                np.repeat(text_regions_p[:, :, np.newaxis], 3, axis=2),
                                num_col_classifier,
                                self.tables,
                        ...
                        _, _, matrix_of_lines_ch_d, splitter_y_new_d, _ = (
                            find_number_of_columns_in_document(
                                np.repeat(
                                    text_regions_p_1_n[:, :, np.newaxis], 3, axis=2
                                ),
                                num_col_classifier,
                                self.tables,
                                pixel_lines,
                ...
                if num_col_classifier >= 3:
                    if np.abs(slope_deskew) < SLOPE_THRESHOLD:
                        ...
                        regions_without_separators_d = (
                            regions_without_separators_d.astype(np.uint8)
                        )
                        regions_without_separators_d = cv2.erode(
                            regions_without_separators_d[:, :], KERNEL, iterations=6
                        )
