...
class EynollahPlotter:
    ...
    def save_plot_of_layout_main(self, text_regions_p, image_page):
        if self.dir_of_layout is not None:
            ...
            patches = [
                mpatches.Patch(
                    color=colors[np.where(values == i)[0][0]],
                    label="{l}".format(
                        l=pixels[int(np.where(values_indexes == i)[0][0])]
                    ),
                )
                for i in values
            ]
            ...
            plt.savefig(
                os.path.join(
                    self.dir_of_layout, self.image_filename_stem + "_layout_main.png"
                )
            )
    ...
    def save_plot_of_layout_main_all(self, text_regions_p, image_page):
        if self.dir_of_all is not None:
            ...
            plt.savefig(
                os.path.join(
                    self.dir_of_all,
                    self.image_filename_stem + "_layout_main_and_page.png",
                )
            )
    ...
    def save_plot_of_layout(self, text_regions_p, image_page):
        if self.dir_of_layout is not None:
            ...
            plt.savefig(
                os.path.join(
                    self.dir_of_layout, self.image_filename_stem + "_layout.png"
                )
            )
    ...
    def save_plot_of_layout_all(self, text_regions_p, image_page):
        if self.dir_of_all is not None:
            ...
            plt.savefig(
                os.path.join(
                    self.dir_of_all, self.image_filename_stem + "_layout_and_page.png"
                )
            )
    ...
    def save_plot_of_textlines(self, textline_mask_tot_ea, image_page):
        if self.dir_of_all is not None:
            ...
            plt.savefig(
                os.path.join(
                    self.dir_of_all, self.image_filename_stem + "_textline_and_page.png"
                )
            )
    ...
    def save_deskewed_image(self, slope_deskew):
        if self.dir_of_all is not None:
            cv2.imwrite(
                os.path.join(self.dir_of_all, self.image_filename_stem + "_org.png"),
                self.image_org,
            )
        if self.dir_of_deskewed is not None:
            ...
            cv2.imwrite(
                os.path.join(
                    self.dir_of_deskewed, self.image_filename_stem + "_deskewed.png"
                ),
                img_rotated,
            )
    ...
    def save_page_image(self, image_page):
        if self.dir_of_all is not None:
            cv2.imwrite(
                os.path.join(self.dir_of_all, self.image_filename_stem + "_page.png"),
                image_page,
            )
        if self.dir_save_page is not None:
            cv2.imwrite(
                os.path.join(
                    self.dir_save_page, self.image_filename_stem + "_page.png"
                ),
                image_page,
            )
    ...
    def save_enhanced_image(self, img_res):
        cv2.imwrite(
            os.path.join(self.dir_out, self.image_filename_stem + "_enhanced.png"),
            img_res,
        )
    ...
    def save_plot_of_textline_density(self, img_patch_org):
        if self.dir_of_all is not None:
            ...
            plt.subplot(1, 2, 2)
            plt.plot(
                gaussian_filter1d(img_patch_org.sum(axis=1), 3),
                np.array(range(len(gaussian_filter1d(img_patch_org.sum(axis=1), 3)))),
                linewidth=8,
            )
            plt.xlabel(
                "Density of textline prediction in direction of X axis", fontsize=60
            )
            ...
            plt.savefig(
                os.path.join(
                    self.dir_of_all,
                    self.image_filename_stem + "_density_of_textline.png",
                )
            )
    ...
    def save_plot_of_rotation_angle(self, angels, var_res):
        if self.dir_of_all is not None:
            ...
            plt.ylabel(
                "variance of sum of rotated textline in direction of x axis",
                fontsize=50,
            )
            plt.plot(
                angels[np.argmax(var_res)],
                var_res[np.argmax(np.array(var_res))],
                "*",
                markersize=50,
                label="Angle of deskewing="
                + str("{:.2f}".format(angels[np.argmax(var_res)]))
                + r"$\degree$",
            )
            plt.legend(loc="best")
            plt.savefig(
                os.path.join(
                    self.dir_of_all, self.image_filename_stem + "_rotation_angle.png"
                )
            )
    ...
    def write_images_into_directory(self, img_contours, image_page):
        if self.dir_of_cropped_images is not None:
            ...
            for cont_ind in img_contours:
                ...
                croped_page = resize_image(
                    croped_page,
                    int(croped_page.shape[0] / self.scale_y),
                    int(croped_page.shape[1] / self.scale_x),
                )

                path = os.path.join(
                    self.dir_of_cropped_images,
                    self.image_filename_stem + "_" + str(index) + ".jpg",
                )
