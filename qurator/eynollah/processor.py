...
class EynollahProcessor(Processor):
    ...
    def process(self):
        ...
        for n, input_file in enumerate(self.input_files):
            ...
            LOG.debug(
                "width %s height %s",
                pcgts.get_Page().imageWidth,
                pcgts.get_Page().imageHeight,
            )
            self.add_metadata(pcgts)
            page = pcgts.get_Page()
            # XXX loses DPI information
            # page_image, _, _ = self.workspace.image_from_page(page, page_id, feature_filter='binarized')
            image_filename = self.workspace.download_file(
                next(self.workspace.mets.find_files(url=page.imageFilename))
            ).local_filename
            eynollah_kwargs = {
