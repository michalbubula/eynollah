...
def return_x_start_end_mothers_childs_and_type_of_reading_order(
        ...
        if type(all_args_uniq[0]) is list:
            for dd in range(len(all_args_uniq)):
                if len(all_args_uniq[dd]) == 2:
                    ...
                    # print('burda2')
                    if (
                        x_s_same_hor[0] == (x_e_same_hor[1] - 1)
                        or x_s_same_hor[1] == (x_e_same_hor[0] - 1)
                        and x_s_same_hor[0] != x_s_same_hor[1]
                        and x_e_same_hor[0] != x_e_same_hor[1]
                    ):
                        # print('burda3')
    ...
    args_lines_not_unified = list(
        set(range(len(y_sep_returned))) - set(args_to_be_unified)
    )

    # print(args_lines_not_unified,'args_lines_not_unified')

    x_start_returned_not_unified = list(
        np.array(x_start_returned)[args_lines_not_unified]
    )
    ...
    y_diff_returned_not_unified = list(
        np.array(y_diff_returned)[args_lines_not_unified]
    )
    ...
    if len(remained_sep_indexes) > 1:
        ...
        remained_sep_indexes_without_mother = np.array(list(remained_sep_indexes))[
            np.array(mother) == 0
        ]
        remained_sep_indexes_with_child_without_mother = np.array(
            list(remained_sep_indexes)
        )[(np.array(mother) == 0) & (np.array(child) == 1)]
        # print(remained_sep_indexes_without_mother,'remained_sep_indexes_without_mother')

        x_end_with_child_without_mother = np.array(x_end)[
            np.array(remained_sep_indexes_with_child_without_mother)
        ]

        x_start_with_child_without_mother = np.array(x_start)[
            np.array(remained_sep_indexes_with_child_without_mother)
        ]

        y_lines_with_child_without_mother = np.array(y_sep)[
            np.array(remained_sep_indexes_with_child_without_mother)
        ]

        reading_orther_type = 0

        x_end_without_mother = np.array(x_end)[
            np.array(remained_sep_indexes_without_mother)
        ]
        x_start_without_mother = np.array(x_start)[
            np.array(remained_sep_indexes_without_mother)
        ]
        y_lines_without_mother = np.array(y_sep)[
            np.array(remained_sep_indexes_without_mother)
        ]
        ...
        if len(remained_sep_indexes_without_mother) >= 2:
            for i in range(len(remained_sep_indexes_without_mother) - 1):
                ...
                nodes_i = set(
                    range(
                        x_start[remained_sep_indexes_without_mother[i]],
                ...
                for j in range(i + 1, len(remained_sep_indexes_without_mother)):
                    ...
                    nodes_j = set(
                        range(
                            x_start[remained_sep_indexes_without_mother[j]],
...
def otsu_copy_binary(img):
    ...
    retval1, threshold1 = cv2.threshold(
        img1, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU
    )
...
def find_features_of_lines(contours_main):
    ...
    areas_main = np.array(
        [cv2.contourArea(contours_main[j]) for j in range(len(contours_main))]
    )
    M_main = [cv2.moments(contours_main[j]) for j in range(len(contours_main))]
    cx_main = [
        (M_main[j]["m10"] / (M_main[j]["m00"] + 1e-32)) for j in range(len(M_main))
    ]
    cy_main = [
        (M_main[j]["m01"] / (M_main[j]["m00"] + 1e-32)) for j in range(len(M_main))
    ]
    x_min_main = np.array(
        [np.min(contours_main[j][:, 0, 0]) for j in range(len(contours_main))]
    )
    x_max_main = np.array(
        [np.max(contours_main[j][:, 0, 0]) for j in range(len(contours_main))]
    )

    y_min_main = np.array(
        [np.min(contours_main[j][:, 0, 1]) for j in range(len(contours_main))]
    )
    y_max_main = np.array(
        [np.max(contours_main[j][:, 0, 1]) for j in range(len(contours_main))]
    )
...
def boosting_headers_by_longshot_region_segmentation(
    textregion_pre_p, textregion_pre_np, img_only_text
):
    ...
    textregion_pre_p[:, :, 0][
        (headers_in_longshot[:, :] == 1) & (textregion_pre_p[:, :, 0] != 4)
    ] = 2
...
def find_num_col(
    regions_without_separators, num_col_classifier, tables, multiplier=3.8
):
    regions_without_separators_0 = regions_without_separators[:, :].sum(axis=0)
    # plt.plot(regions_without_separators_0)
    # plt.show()
    sigma_ = 35  # 70#35
    meda_n_updown = regions_without_separators_0[
        len(regions_without_separators_0):: -1
    ]
    first_nonzero = next(
        (i for i, x in enumerate(regions_without_separators_0) if x), 0
    )
    ...
    peaks = peaks[
        (peaks > 0.06 * regions_without_separators.shape[1])
        & (peaks < 0.94 * regions_without_separators.shape[1])
    ]
    peaks_neg = peaks_neg[
        (peaks_neg > 370) & (peaks_neg < (regions_without_separators.shape[1] - 370))
    ]
    ...
    if not tables:
        if (num_col_classifier - ((len(interest_neg_fin)) + 1)) >= 3:
            ...
            interest_neg_fin_sorted = np.array(interest_neg_fin)[
                index_sort_interest_neg_fin
            ]
    ...
    if num_col == 3:
        if (
            (peaks_neg_fin[0] > p_g_u and peaks_neg_fin[1] > p_g_u)
            or (peaks_neg_fin[0] < p_g_l and peaks_neg_fin[1] < p_g_l)
            or ((peaks_neg_fin[0] + 200) < p_m and peaks_neg_fin[1] < p_m)
            or ((peaks_neg_fin[0] - 200) > p_m and peaks_neg_fin[1] > p_m)
        ):
            num_col = 1
            peaks_neg_fin = []
    ...
    if num_col == 3:
        if (
            (peaks_neg_true[0] > p_g_u and peaks_neg_true[1] > p_g_u)
            or (peaks_neg_true[0] < p_g_l and peaks_neg_true[1] < p_g_l)
            or (peaks_neg_true[0] < p_m and (peaks_neg_true[1] + 200) < p_m)
            or ((peaks_neg_true[0] - 200) > p_m and peaks_neg_true[1] > p_m)
        ):
            num_col = 1
            peaks_neg_true = []
        ...
        elif (peaks_neg_true[1] < p_g_u and peaks_neg_true[1] > p_g_l) and (
...
def find_num_col_only_image(regions_without_separators, multiplier=3.8):
    ...
    peaks = peaks[
        (peaks > 0.09 * regions_without_separators.shape[1])
        & (peaks < 0.91 * regions_without_separators.shape[1])
    ]

    peaks_neg = peaks_neg[
        (peaks_neg > 500) & (peaks_neg < (regions_without_separators.shape[1] - 500))
    ]
    # print(peaks)
    interest_pos = z[peaks]
    ...
    if num_col == 3:
        if (
            (peaks_neg_fin[0] > p_g_u and peaks_neg_fin[1] > p_g_u)
            or (peaks_neg_fin[0] < p_g_l and peaks_neg_fin[1] < p_g_l)
            or (peaks_neg_fin[0] < p_m and peaks_neg_fin[1] < p_m)
            or (peaks_neg_fin[0] > p_m and peaks_neg_fin[1] > p_m)
        ):
            num_col = 1
        else:
    ...
    if num_col == 3:
        if (
            (peaks_neg_true[0] > p_g_u and peaks_neg_true[1] > p_g_u)
            or (peaks_neg_true[0] < p_g_l and peaks_neg_true[1] < p_g_l)
            or (peaks_neg_true[0] < p_m and peaks_neg_true[1] < p_m)
            or (peaks_neg_true[0] > p_m and peaks_neg_true[1] > p_m)
        ):
            num_col = 1
            peaks_neg_true = []
    ...
    if num_col == 4:
        if (
            len(np.array(peaks_neg_true)[np.array(peaks_neg_true) < p_g_l]) == 2
            or len(
                np.array(peaks_neg_true)[np.array(peaks_neg_true) > (len(y) - p_g_l)]
            )
            == 2
        ):
            num_col = 1
...
def put_drop_out_from_only_drop_model(layout_no_patch, layout1):
    ...
    for jj in range(len(contours_drop_parent)):
        ...
        if (
            ((map_of_drop_contour_bb == 1) * 1).sum()
            / float(((map_of_drop_contour_bb == 5) * 1).sum())
            * 100
        ) >= 15:
    ...
    layout_no_patch = cv2.fillPoly(
        layout_no_patch, pts=contours_drop_parent_final, color=(4, 4, 4)
    )
...
def check_any_text_region_in_model_one_is_main_or_header(
    ...
    for ii in range(len(contours_only_text_parent)):
        ...
        pixels_header = (
            ((img[:, :, 0] == 255) & (regions_model_full[:, :, 0] == 2)) * 1
        ).sum()
        pixels_main = all_pixels - pixels_header

        if (pixels_header >= pixels_main) and (
            (length_con[ii] / float(height_con[ii])) >= 1.3
        ):
            regions_model_1[:, :][
                (regions_model_1[:, :] == 1) & (img[:, :, 0] == 255)
            ] = 2
            contours_only_text_parent_head.append(con)
            if contours_only_text_parent_d_ordered is not None:
                contours_only_text_parent_head_d.append(
                    contours_only_text_parent_d_ordered[ii]
                )
            ...
            if contours_only_text_parent_d_ordered is not None:
                contours_only_text_parent_main_d.append(
                    contours_only_text_parent_d_ordered[ii]
                )
...
def check_any_text_region_in_model_one_is_main_or_header_light(
    ...
    regions_model_1 = cv2.resize(
        regions_model_1,
        (int(regions_model_1.shape[1] / 3.0), int(regions_model_1.shape[0] / 3.0)),
        interpolation=cv2.INTER_NEAREST,
    )
    regions_model_full = cv2.resize(
        regions_model_full,
        (
            int(regions_model_full.shape[1] / 3.0),
            int(regions_model_full.shape[0] / 3.0),
        ),
        interpolation=cv2.INTER_NEAREST,
    )
    contours_only_text_parent = [
        (i / 3.0).astype(np.int32) for i in contours_only_text_parent
    ]
    ...
    regions_model_1 = cv2.resize(
        regions_model_1, (w_o, h_o), interpolation=cv2.INTER_NEAREST
    )
    ...
    contours_only_text_parent_head = [
        (i * 3.0).astype(np.int32) for i in contours_only_text_parent_head
    ]
    contours_only_text_parent_main = [
        (i * 3.0).astype(np.int32) for i in contours_only_text_parent_main
    ]
    ###
...
def small_textlines_to_parent_adherence2(textlines_con, textline_iamge, num_col):
    ...
    for m1 in range(len(textlines_con)):
        ...
        # plt.show()
        areas_cnt_text = np.array(
            [cv2.contourArea(textlines_tot[j]) for j in range(len(textlines_tot))]
        )
        areas_cnt_text = areas_cnt_text / float(
            textline_iamge.shape[0] * textline_iamge.shape[1]
        )
        ...
        img_textline_s = cv2.fillPoly(
            img_textline_s, pts=textlines_small, color=(1, 1, 1)
        )

        img_textline_b = np.zeros((textline_iamge.shape[0], textline_iamge.shape[1]))
        img_textline_b = cv2.fillPoly(
            img_textline_b, pts=textlines_big, color=(1, 1, 1)
        )
        ...
        if sum_intersection_sb > 0:
            ...
            for z1 in range(len(textlines_small)):
                ...
                for z2 in range(len(textlines_big)):
                    ...
                    img_text = cv2.fillPoly(
                        img_text, pts=[textlines_small[z1]], color=(1, 1, 1)
                    )

                    img_text2 = np.zeros(
                        (textline_iamge.shape[0], textline_iamge.shape[1])
                    )
                    img_text2 = cv2.fillPoly(
                        img_text2, pts=[textlines_big[z2]], color=(1, 1, 1)
                    )
            ...
            smalls_list = np.array(dis_small_from_bigs_tot)[
                np.array(dis_small_from_bigs_tot) >= 0
            ]
            ...
            for z in list(set(smalls_list)):
                index_small_textlines = list(
                    np.where(np.array(dis_small_from_bigs_tot) == z)[0]
                )
                # print(z,index_small_textlines)

                img_text2 = np.zeros(
                    (textline_iamge.shape[0], textline_iamge.shape[1], 3)
                )
                img_text2 = cv2.fillPoly(
                    img_text2, pts=[textlines_big[z]], color=(255, 255, 255)
                )
                ...
                for k in index_small_textlines:
                    img_text2 = cv2.fillPoly(
                        img_text2, pts=[textlines_small[k]], color=(255, 255, 255)
                    )
...
def order_of_regions(textline_mask, contours_main, contours_header, y_ref):
    ...
    if len(contours_header) is not None:
        areas_header = np.array(
            [cv2.contourArea(contours_header[j]) for j in range(len(contours_header))]
        )
        M_header = [
            cv2.moments(contours_header[j]) for j in range(len(contours_header))
        ]
        cx_header = [
            (M_header[j]["m10"] / (M_header[j]["m00"] + 1e-32))
            for j in range(len(M_header))
        ]
        cy_header = [
            (M_header[j]["m01"] / (M_header[j]["m00"] + 1e-32))
            for j in range(len(M_header))
        ]

        x_min_header = np.array(
            [np.min(contours_header[j][:, 0, 0]) for j in range(len(contours_header))]
        )
        x_max_header = np.array(
            [np.max(contours_header[j][:, 0, 0]) for j in range(len(contours_header))]
        )

        y_min_header = np.array(
            [np.min(contours_header[j][:, 0, 1]) for j in range(len(contours_header))]
        )
        y_max_header = np.array(
            [np.max(contours_header[j][:, 0, 1]) for j in range(len(contours_header))]
        )
        # print(cy_main,'mainy')
    ...
    for i in range(len(peaks_neg_new) - 1):
        ...
        indexes_in = matrix_of_orders[:, 0][
            (matrix_of_orders[:, 3] >= top) & (matrix_of_orders[:, 3] < down)
        ]
        cxs_in = matrix_of_orders[:, 2][
            (matrix_of_orders[:, 3] >= top) & (matrix_of_orders[:, 3] < down)
        ]
        cys_in = matrix_of_orders[:, 3][
            (matrix_of_orders[:, 3] >= top) & (matrix_of_orders[:, 3] < down)
        ]
        types_of_text = matrix_of_orders[:, 1][
            (matrix_of_orders[:, 3] >= top) & (matrix_of_orders[:, 3] < down)
        ]
        index_types_of_text = matrix_of_orders[:, 4][
            (matrix_of_orders[:, 3] >= top) & (matrix_of_orders[:, 3] < down)
        ]
...
def combine_hor_lines_and_delete_cross_points_and_get_lines_features_back_new(
    ...
    contours_lines_ver, hierarchy = cv2.findContours(
        thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    for i in range(len(x_min_main_ver)):
        img_p_in_ver[
            int(y_min_main_ver[i]): int(y_min_main_ver[i]) + 30,
            int(cx_main_ver[i]) - 25: int(cx_main_ver[i]) + 25,
            0,
        ] = 0
        img_p_in_ver[
            int(y_max_main_ver[i]) - 30: int(y_max_main_ver[i]),
            int(cx_main_ver[i]) - 25: int(cx_main_ver[i]) + 25,
            0,
        ] = 0
    ...
    contours_lines_hor, hierarchy = cv2.findContours(
        thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    x_width_smaller_than_acolumn_width = img_in_hor.shape[1] / float(
        num_col_classifier + 1.0
    )
    ...
    if len_lines_bigger_than_x_width_smaller_than_acolumn_width_per_column < 10:
            ...
            if type(all_args_uniq[0]) is list:
                ...
                for dd in range(len(all_args_uniq)):
                    ...
                    for jv in range(len(some_args)):
                        ...
                        img_p_in = cv2.fillPoly(
                            img_in_hor,
                            pts=[contours_lines_hor[some_args[jv]]],
                            color=(1, 1, 1),
                        )

                        if any(
                            i_diff > (img_p_in_ver.shape[1] / float(3.3))
                            for i_diff in diff_x_some
                        ):
                            img_p_in[
                                int(np.mean(some_cy)) - 5: int(np.mean(some_cy)) + 5,
                                int(np.min(some_x_min)): int(np.max(some_x_max)),
                            ] = 1

                    sum_dis = dist_x_hor[some_args].sum()
                    diff_max_min_uniques = np.max(x_max_main_hor[some_args]) - np.min(
                        x_min_main_hor[some_args]
                    )

                    if (
                        diff_max_min_uniques > sum_dis
                        and ((sum_dis / float(diff_max_min_uniques)) > 0.85)
                        and (
                            (diff_max_min_uniques / float(img_p_in_ver.shape[1])) > 0.85
                        )
                        and np.std(dist_x_hor[some_args])
                        < (0.55 * np.mean(dist_x_hor[some_args]))
                    ):
        ...
        ret, thresh = cv2.threshold(imgray, 0, 255, 0)
        contours_cross, _ = cv2.findContours(
            thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )

        cx_cross, cy_cross, _, _, _, _, _ = find_new_features_of_contours(
            contours_cross
        )

        for ii in range(len(cx_cross)):
            img_p_in[
                int(cy_cross[ii]) - 30: int(cy_cross[ii]) + 30,
                int(cx_cross[ii]) + 5: int(cx_cross[ii]) + 40,
                0,
            ] = 0
            img_p_in[
                int(cy_cross[ii]) - 30: int(cy_cross[ii]) + 30,
                int(cx_cross[ii]) - 40: int(cx_cross[ii]) - 4,
                0,
            ] = 0
...
def find_number_of_columns_in_document(
    region_pre_p, num_col_classifier, tables, pixel_lines, contours_h=None
):
    ...
    separators_closeup_new = np.zeros(
        (separators_closeup.shape[0], separators_closeup.shape[1])
    )
    ...
    separators_closeup_n_binary = np.zeros(
        (separators_closeup_n.shape[0], separators_closeup_n.shape[1])
    )
    ...
    # print('burda3')
    contours_line_e, hierarchy_e = cv2.findContours(
        thresh_e, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    ...
    separators_closeup_n_binary = cv2.fillPoly(
        separators_closeup_n_binary, pts=cnts_hor_e, color=(0, 0, 0)
    )
    ...
    contours_line_vers, hierarchy = cv2.findContours(
        thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    (
        slope_lines,
    ...
    contours_line_hors, hierarchy = cv2.findContours(
        thresh, cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
    )
    (
        slope_lines,
    ...
    matrix_of_lines_ch[: len(cy_main_hor), 2] = (
        x_min_main_hor + 50
    )  # x_min_main_hor+150
    matrix_of_lines_ch[len(cy_main_hor):, 2] = x_min_main_ver

    matrix_of_lines_ch[: len(cy_main_hor), 3] = (
        x_max_main_hor - 50
    )  # x_max_main_hor-150
    ...
    if contours_h is not None:
        ...
        matrix_l_n = np.zeros(
            (
                matrix_of_lines_ch.shape[0] + len(cy_main_head),
                matrix_of_lines_ch.shape[1],
            )
        )
        ...
        matrix_l_n[matrix_of_lines_ch.shape[0]:, 7] = (
            y_max_main_head  # y_min_main_head+1-8
        )
    ...
    cy_main_splitters = cy_main_hor[
        (x_min_main_hor <= 0.16 * region_pre_p.shape[1])
        & (x_max_main_hor >= 0.84 * region_pre_p.shape[1])
    ]
    ...
    if contours_h is not None:
        try:
            cy_main_splitters_head = cy_main_head[
                (x_min_main_head <= 0.16 * region_pre_p.shape[1])
                & (x_max_main_head >= 0.84 * region_pre_p.shape[1])
            ]
            cy_main_splitters = np.array(
                list(cy_main_splitters) + list(cy_main_splitters_head)
            )
        except:
            pass
    ...
    for itiles in args_big_parts:
        ...
        try:
            num_col, peaks_neg_fin = find_num_col(
                regions_without_separators_tile,
                num_col_classifier,
                tables,
                multiplier=7.0,
            )
...
def return_boxes_of_images_by_order_of_reading_new(
    ...
    for i in range(len(splitter_y_new) - 1):
        ...
        matrix_new = matrix_of_lines_ch[:, :][
            (matrix_of_lines_ch[:, 6] > splitter_y_new[i])
        ...
        if (
            1 > 0
        ):  # len( matrix_new[:,9][matrix_new[:,9]==1] )>0 and np.max(matrix_new[:,8][matrix_new[:,9]==1])>=0.1*(np.abs(splitter_y_new[i+1]-splitter_y_new[i] )):
            ...
            try:
                ...
                if (
                    len(peaks_neg_fin) + 1
                ) < num_col_classifier or num_col_classifier == 6:
                    # print('burda')
                    ...
                    for i_n in range(len(peaks_neg_fin_early) - 1):
                        ...
                        try:
                            num_col, peaks_neg_fin1 = find_num_col(
                                regions_without_separators[
                                    int(splitter_y_new[i]): int(splitter_y_new[i + 1]),
                                    peaks_neg_fin_early[i_n]: peaks_neg_fin_early[
                                        i_n + 1
                                    ],
                                ],
                                num_col_classifier,
                        ...
                        peaks_neg_fin = list(
                            np.array(peaks_neg_fin) + peaks_neg_fin_early[i_n]
                        )
            ...
            if right2left_readingorder:
                x_max_hor_some_new = (
                    regions_without_separators.shape[1] - x_min_hor_some
                )
                x_min_hor_some_new = (
                    regions_without_separators.shape[1] - x_max_hor_some
                )

                x_min_hor_some = list(np.copy(x_min_hor_some_new))
                x_max_hor_some = list(np.copy(x_max_hor_some_new))

            peaks_neg_tot = return_points_with_boundies(
                peaks_neg_fin, 0, regions_without_separators[:, :].shape[1]
            )
            ...
            if (reading_order_type == 1) or (
                ...
                and (len(y_lines_without_mother) >= 2 or there_is_sep_with_child == 1)
            ):
                ...
                try:
                    ...
                    y_type_2_up = np.array(y_type_2)[
                        (np.array(y_type_2) > int(splitter_y_new[i]))
                        & (np.array(y_type_2) <= y_grenze)
                    ]
                    ...
                    if len(y_type_2_up) > 0:
                        ...
                        if len(y_diff_main_separator_up) > 0:
                            args_to_be_kept = np.array(
                                list(set(args_early_ys) - set(args_main_to_deleted))
                            )
                            # print(args_to_be_kept,'args_to_be_kept')
                            boxes.append(
                                [
                                    0,
                                    peaks_neg_tot[len(peaks_neg_tot) - 1],
                            ...
                            for ij in range(len(x_starting_up)):
                                nodes_in = nodes_in + list(
                                    np.array(range(x_starting_up[ij], x_ending_up[ij]))
                                )

                            # print(np.unique(nodes_in),'nodes_in')

                            if set(np.unique(nodes_in)) == set(
                                np.array(range(len(peaks_neg_tot) - 1))
                            ):
                                pass
                            elif set(np.unique(nodes_in)) == set(
                                np.array(range(1, len(peaks_neg_tot) - 1))
                            ):
                                pass
                            else:
                                # print('burdaydikh')
                                args_to_be_kept2 = np.array(
                                    list(set(args_early_ys2) - set(args_up2))
                                )
                            ...
                            if set(np.unique(nodes_in)) == set(
                                ...
                                args_to_be_kept2 = np.array(
                                    list(set(args_early_ys) - set(args_up))
                                )
                    ...
                    if (
                        ...
                        and reading_order_type == 0
                    ) or reading_order_type == 1:
                            ...
                            for dj in range(len(x_start_without_mother)):
                                columns_covered_by_mothers = (
                                    ...
                                    + list(
                                        np.array(
                                            range(
                                                x_start_without_mother[dj],
                                                x_end_without_mother[dj],
                                            )
                                        )
                                    )
                                )
                            columns_covered_by_mothers = list(
                                set(columns_covered_by_mothers)
                            )

                            all_columns = np.array(range(len(peaks_neg_tot) - 1))

                            columns_not_covered = list(
                                set(all_columns) - set(columns_covered_by_mothers)
                            )
                        ...
                        for dj in range(len(x_end_with_child_without_mother)):
                            columns_covered_by_with_child_no_mothers = (
                                ...
                                + list(
                                    np.array(
                                        range(
                                            x_start_with_child_without_mother[dj],
                                            x_end_with_child_without_mother[dj],
                                        )
                                    )
                                )
                            )
                        columns_covered_by_with_child_no_mothers = list(
                            set(columns_covered_by_with_child_no_mothers)
                        )
                        ...
                        for i_s in range(len(x_end_with_child_without_mother)):
                            columns_not_covered_child_no_mother.append(
                                x_start_with_child_without_mother[i_s]
                            )

                        columns_not_covered_child_no_mother = np.sort(
                        ...
                        for i_s_nc in columns_not_covered_child_no_mother:
                            if i_s_nc in x_start_with_child_without_mother:
                                x_end_biggest_column = np.array(
                                    x_end_with_child_without_mother
                                )[
                                    np.array(x_start_with_child_without_mother)
                                    == i_s_nc
                                ][
                                    0
                                ]
                                ...
                                args_all_biggest_lines = np.array(
                                    args_all_biggest_lines
                                )
                                ...
                                for i_c in range(len(y_column_nc)):
                                    ...
                                    y_all_between_nm_wc = y_type_2[
                                        ind_all_lines_betweeen_nm_wc
                                    ]
                                    x_starting_all_between_nm_wc = x_starting[
                                        ind_all_lines_betweeen_nm_wc
                                    ]
                                    x_ending_all_between_nm_wc = x_ending[
                                        ind_all_lines_betweeen_nm_wc
                                    ]

                                    x_diff_all_between_nm_wc = (
                                    ...
                                    all_columns = np.array(
                                        range(i_s_nc, x_end_biggest_column)
                                    )
                                    ...
                                    if len(x_diff_all_between_nm_wc) > 0 and set(
                                        list(
                                            np.array(
                                                range(
                                                    x_starting_all_between_nm_wc[
                                        ...
                                        if (
                                            len(index_lines_so_close_to_top_separator)
                                            > 0
                                        ):
                                            indexes_remained_after_deleting_closed_lines = np.array(
                                                list(
                                                    set(
                                                        list(
                                                            np.array(
                                        ...
                                        x_starting_all_between_nm_wc = list(
                                            x_starting_all_between_nm_wc
                                        )
                                        x_ending_all_between_nm_wc = list(
                                            x_ending_all_between_nm_wc
                                        )

                                        y_all_between_nm_wc.append(y_column_nc[i_c])
                                        x_starting_all_between_nm_wc.append(i_s_nc)
                                        x_ending_all_between_nm_wc.append(
                                    ...
                                    if len(x_diff_all_between_nm_wc) > 0:
                                        try:
                                            x_starting_all_between_nm_wc.append(
                                                x_starting_all_between_nm_wc[biggest]
                                            )
                                            x_ending_all_between_nm_wc.append(
                                                x_ending_all_between_nm_wc[biggest]
                                            )
                                    ...
                                    x_starting_all_between_nm_wc = np.array(
                                        x_starting_all_between_nm_wc
                                    )
                                    x_ending_all_between_nm_wc = np.array(
                                        x_ending_all_between_nm_wc
                                    )

                                    ind_args_between = np.array(
                                        range(len(x_ending_all_between_nm_wc))
                                    )

                                    for column in range(i_s_nc, x_end_biggest_column):
                                        ind_args_in_col = ind_args_between[
                                            x_starting_all_between_nm_wc == column
                                        ]
                                        # print('babali2')
                                        # print(ind_args_in_col,'ind_args_in_col')
                                        ind_args_in_col = np.array(ind_args_in_col)
                                        ...
                                        x_start_column = x_starting_all_between_nm_wc[
                                            ind_args_in_col
                                        ]
                                        x_end_column = x_ending_all_between_nm_wc[
                                            ind_args_in_col
                                        ]
                                        # print('babali3')
                                        ind_args_col_sorted = np.argsort(y_column)
                                        y_col_sort = y_column[ind_args_col_sorted]
                                        x_start_column_sort = x_start_column[
                                            ind_args_col_sorted
                                        ]
                                        x_end_column_sort = x_end_column[
                                            ind_args_col_sorted
                                        ]
                                        # print('babali4')
                                        for ii in range(len(y_col_sort)):
                                            # print('babali5')
                                            y_lines_by_order.append(y_col_sort[ii])
                                            x_start_by_order.append(
                                                x_start_column_sort[ii]
                                            )
                                            x_end_by_order.append(
                                                x_end_column_sort[ii] - 1
                                            )
                    ...
                    for il in range(len(y_lines_by_order)):
                        ...
                        for column in range(x_start_itself, x_end_itself + 1):
                            ...
                            for yic in range(len(y_copy)):
                                ...
                                if (
                                        y_copy[yic] > y_itself
                                        and x_start_copy[yic] <= column <= x_end_copy[yic]
                                ):
                ...
                if len(x_starting) > 0:
                    ...
                    for dj in range(len(x_starting)):
                        if set(
                            list(np.array(range(x_starting[dj], x_ending[dj])))
                        ) == set(all_columns):
                            pass
                        else:
                            columns_covered_by_lines_covered_more_than_2col = (
    ...
    if right2left_readingorder:
        ...
        if len(peaks_neg_tot_tables) >= 1:
            for peaks_tab_ind in peaks_neg_tot_tables:
                peaks_neg_tot_tables_ind = regions_without_separators.shape[
                    1
                ] - np.array(peaks_tab_ind)
