...
def filter_contours_area_of_image(image, contours, hierarchy, max_area, min_area):
    ...
    for jv, c in enumerate(contours):
        ...
        area = polygon.area
        if (
            min_area * np.prod(image.shape[:2]) <= area <= max_area * np.prod(image.shape[:2])
...
def filter_contours_area_of_image_tables(
    image, contours, hierarchy, max_area, min_area
):
    found_polygons_early = list()
    ...
    for jv, c in enumerate(contours):
        ...
        if min_area * np.prod(image.shape[:2]) <= area <= max_area * np.prod(
            image.shape[:2]
        ):  # and hierarchy[0][jv][3]==-1 :
            # print(c[0][0][1])
            found_polygons_early.append(
...
def find_new_features_of_contours(contours_main):
    ...
    try:
        ...
        argmin_x_main = np.array(
            [np.argmin(contours_main[j][:, 0, 0]) for j in range(len(contours_main))]
        )

        x_min_from_argmin = np.array(
            [
                contours_main[j][argmin_x_main[j], 0, 0]
                for j in range(len(contours_main))
            ]
        )
        y_corr_x_min_from_argmin = np.array(
            [
                contours_main[j][argmin_x_main[j], 0, 1]
                for j in range(len(contours_main))
            ]
        )
        ...
        x_min_main = np.array(
            [np.min(contours_main[j][:, 0]) for j in range(len(contours_main))]
        )

        argmin_x_main = np.array(
            [np.argmin(contours_main[j][:, 0]) for j in range(len(contours_main))]
        )

        x_min_from_argmin = np.array(
            [contours_main[j][argmin_x_main[j], 0] for j in range(len(contours_main))]
        )
        y_corr_x_min_from_argmin = np.array(
            [contours_main[j][argmin_x_main[j], 1] for j in range(len(contours_main))]
        )

        x_max_main = np.array(
            [np.max(contours_main[j][:, 0]) for j in range(len(contours_main))]
        )

        y_min_main = np.array(
            [np.min(contours_main[j][:, 1]) for j in range(len(contours_main))]
        )
        y_max_main = np.array(
            [np.max(contours_main[j][:, 1]) for j in range(len(contours_main))]
        )
...
def return_contours_of_interested_region(region_pre_p, pixel, min_area=0.0002):
    ...
    contours_imgs = filter_contours_area_of_image_tables(
        thresh, contours_imgs, hierarchy, max_area=1, min_area=min_area
    )

    return contours_imgs


def do_work_of_contours_in_image(
    queue_of_all_params, contours_per_process, indexes_r_con_per_pro, img, slope_first
):
    ...
    for mv in range(len(contours_per_process)):
        ...
        img_copy = cv2.fillPoly(
            img_copy, pts=[contours_per_process[mv]], color=(1, 1, 1)
        )
        ...
        cont_int[0][:, 0, 0] = cont_int[0][:, 0, 0] + np.abs(
            img_copy.shape[1] - img.shape[1]
        )
        cont_int[0][:, 0, 1] = cont_int[0][:, 0, 1] + np.abs(
            img_copy.shape[0] - img.shape[0]
        )

        cnts_org_per_each_subprocess.append(cont_int[0])

    queue_of_all_params.put(
        [cnts_org_per_each_subprocess, index_by_text_region_contours]
    )
...
def get_textregion_contours_in_org_image_multi(cnts, img, slope_first):
    ...
    for i in range(num_cores):
        ...
        processes.append(
            Process(
                target=do_work_of_contours_in_image,
                args=(
                    queue_of_all_params,
...
def get_textregion_contours_in_org_image_multi2(cnts, img, slope_first):
    ...
    with Pool(cpu_count()) as p:
        cnts_org = p.starmap(
            loop_contour_image,
            [(index_l, cnts, img, slope_first) for index_l in range(len(cnts))],
        )
...
def get_textregion_contours_in_org_image_light(cnts, img, slope_first):
    ...
    img = cv2.resize(
        img,
        (int(img.shape[1] / 3.0), int(img.shape[0] / 3.0)),
...
def return_contours_of_interested_region_by_min_size(
    region_pre_p, pixel, min_size=0.00003
):
...
def return_contours_of_interested_region_by_size(
    region_pre_p, pixel, min_area, max_area
):
