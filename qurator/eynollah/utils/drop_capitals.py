...
def adhere_drop_capital_region_into_corresponding_textline(
    ...
    cx_h, cy_h, _, _, _, _, _ = find_new_features_of_contours(
        contours_only_text_parent_h
    )
    cx_d, cy_d, _, _, y_min_d, y_max_d, _ = find_new_features_of_contours(
        polygons_of_drop_capitals
    )
    ...
    for j_cont in range(len(contours_only_text_parent)):
        img_con_all[
            all_box_coord[j_cont][0]: all_box_coord[j_cont][1],
            all_box_coord[j_cont][2]: all_box_coord[j_cont][3],
            0,
        ] = (j_cont + 1) * 3
    ...
    for i_drop in range(len(polygons_of_drop_capitals)):
        ...
        img_con = cv2.fillPoly(
            img_con, pts=[polygons_of_drop_capitals[i_drop]], color=(1, 1, 1)
        )
        ...
        if curved_line:
            ...
            if len(region_with_intersected_drop) > 1:
                ...
                for i in range(len(region_with_intersected_drop)):
                    ...
                    sum_pixels_of_intersection.append(
                        (
                            (
                                img_con_all_copy[:, :, 0]
                                == (region_with_intersected_drop[i] * 3 + 1)
                            )
                            * 1
                        ).sum()
                    )
                ...
                region_final = (
                    region_with_intersected_drop[np.argmax(sum_pixels_of_intersection)]
                    - 1
                )
                ...
                try:
                    ...
                    cnt_nearest = np.copy(
                        all_found_textline_polygons[int(region_final)][arg_min]
                    )
                    cnt_nearest[:, 0, 0] = all_found_textline_polygons[
                        int(region_final)
                    ][arg_min][
                        :, 0, 0
                    ]  # +all_box_coord[int(region_final)][2]
                    cnt_nearest[:, 0, 1] = all_found_textline_polygons[
                        int(region_final)
                    ][arg_min][
                        :, 0, 1
                    ]  # +all_box_coord[int(region_final)][0]

                    img_textlines = np.zeros(
                        (text_regions_p.shape[0], text_regions_p.shape[1], 3)
                    )
                    img_textlines = cv2.fillPoly(
                        img_textlines, pts=[cnt_nearest], color=(255, 255, 255)
                    )
                    img_textlines = cv2.fillPoly(
                        img_textlines,
                        pts=[polygons_of_drop_capitals[i_drop]],
                        color=(255, 255, 255),
                    )
                    ...
                    contours_combined, hierarchy = cv2.findContours(
                    ...
                    areas_cnt_text = np.array(
                        [
                            cv2.contourArea(contours_combined[j])
                            for j in range(len(contours_combined))
                        ]
                    )
                ...
                try:
                    ...
                    # print(np.shape(contours_biggest))
                    # print(contours_biggest[:])
                    contours_biggest[:, 0, 0] = contours_biggest[
                        :, 0, 0
                    ]  # -all_box_coord[int(region_final)][2]
                    contours_biggest[:, 0, 1] = contours_biggest[
                        :, 0, 1
                    ]  # -all_box_coord[int(region_final)][0]
            ...
            if len(region_with_intersected_drop) > 1:
                ...
                try:
                    ...
                    cnt_nearest[:, 0] = (
                        all_found_textline_polygons[int(region_final)][arg_min][:, 0]
                        + all_box_coord[int(region_final)][2]
                    )
                    cnt_nearest[:, 1] = (
                        all_found_textline_polygons[int(region_final)][arg_min][:, 1]
                        + all_box_coord[int(region_final)][0]
                    )
                    ...
                    # print(np.shape(contours_biggest))
                    # print(contours_biggest[:])
                    contours_biggest[:, 0, 0] = (
                        contours_biggest[:, 0, 0] - all_box_coord[int(region_final)][2]
                    )
                    contours_biggest[:, 0, 1] = (
                        contours_biggest[:, 0, 1] - all_box_coord[int(region_final)][0]
                    )

                    contours_biggest = contours_biggest.reshape(
                        np.shape(contours_biggest)[0], np.shape(contours_biggest)[2]
                    )
...
def filter_small_drop_capitals_from_no_patch_layout(layout_no_patch, layout1):
    ...
    for jj in range(len(contours_drop_parent)):
        ...
        iou_of_box_and_contoure = (
            float(drop_only.shape[0] * drop_only.shape[1])
            * areas_cnt_text[jj]
            / float(w * h)
            * 100
        )
        ...
        if (
            iou_of_box_and_contoure > 60
            and weigh_to_height_ratio < 1.2
            and height_to_weight_ratio < 2
        ):
