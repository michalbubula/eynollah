...
def dedup_separate_lines(img_patch, contour_text_interest, thetha, axis):
    ...
    rotation_matrix = np.array(
        [[np.cos(thetha), -np.sin(thetha)], [np.sin(thetha), np.cos(thetha)]]
    )
    ...
    if 1 > 0:
        ...
        try:
            ...
            y_padded_up_to_down_padded_e[20: len(y_padded_up_to_down_e) + 20] = (
                y_padded_up_to_down_e
            )
            y_padded_up_to_down_padded_e = gaussian_filter1d(
                y_padded_up_to_down_padded_e, 2
            )
            ...
            if len(arg_diff_cluster) > 0:
                ...
                clusters_to_be_deleted.append(
                    arg_neg_must_be_deleted[
                        arg_diff_cluster[len(arg_diff_cluster) - 1] + 1:
                    ]
                )
            ...
            if len(clusters_to_be_deleted) > 0:
                ...
                for m in range(len(clusters_to_be_deleted)):
                    ...
                    for m1 in range(len(clusters_to_be_deleted[m])):
                        peaks_new = peaks_new[
                            peaks_new != peaks_e[clusters_to_be_deleted[m][m1] - 1]
                        ]
                        peaks_new = peaks_new[
                            peaks_new != peaks_e[clusters_to_be_deleted[m][m1]]
                        ]

                        peaks_neg_new = peaks_neg_new[
                            peaks_neg_new != peaks_neg_e[clusters_to_be_deleted[m][m1]]
                        ]
                peaks_new_tot = []
                for i1 in peaks_new:
            ...
            textline_con_fil = filter_contours_area_of_image(
                img_patch, textline_con, hierarchy, max_area=1, min_area=0.0008
            )
            y_diff_mean = np.mean(
                np.diff(peaks_new_tot)
            )  # self.find_contours_mean_y_diff(textline_con_fil)
    ...
    y_padded_up_to_down_padded = gaussian_filter1d(
        y_padded_up_to_down_padded, sigma_gaus
    )
...
def separate_lines(img_patch, contour_text_interest, thetha, x_help, y_help):
    ...
    try:
        ...
        arg_neg_must_be_deleted = np.array(range(len(peaks_neg)))[
    ...
    try:
        ...
        if len(clusters_to_be_deleted) > 0:
            ...
            for m in range(len(clusters_to_be_deleted)):
                ...
                for m1 in range(len(clusters_to_be_deleted[m])):
                    peaks_new = peaks_new[
                        peaks_new != peaks[clusters_to_be_deleted[m][m1] - 1]
                    ]
                    peaks_new = peaks_new[
                        peaks_new != peaks[clusters_to_be_deleted[m][m1]]
                    ]

                    peaks_neg_new = peaks_neg_new[
                        peaks_neg_new != peaks_neg[clusters_to_be_deleted[m][m1]]
                    ]
            peaks_new_tot = []
            for i1 in peaks_new:
                peaks_new_tot.append(i1)
    ...
    if len(peaks_neg) == len(peaks) + 1 and len(peaks) >= 3:
        for jj in range(len(peaks)):
            ...
            if jj == (len(peaks) - 1):
                ...
                if peaks_values[jj] > mean_value_of_peaks - std_value_of_peaks / 2.0:
                    point_up = (
                        peaks[jj] + first_nonzero - int(1.3 * dis_to_next_up)
                    )  # +int(dis_to_next_up*1./4.0)
                    point_down = (
                        y_max_cont - 1
                    )  # peaks[jj] + first_nonzero + int(1.3 * dis_to_next_down) #point_up# np.max(y_cont)#peaks[jj] + first_nonzero + int(1.4 * dis_to_next_down)  ###-int(dis_to_next_down*1./4.0)
                else:
                    point_up = (
                        peaks[jj] + first_nonzero - int(1.4 * dis_to_next_up)
                    )  # +int(dis_to_next_up*1./4.0)
                    point_down = (
                        y_max_cont - 1
                    )  # peaks[jj] + first_nonzero + int(1.6 * dis_to_next_down) #point_up# np.max(y_cont)#peaks[jj] + first_nonzero + int(1.4 * dis_to_next_down)  ###-int(dis_to_next_down*1./4.0)
                ...
                if peaks_values[jj] > mean_value_of_peaks - std_value_of_peaks / 2.0:
                    point_up = (
                        peaks[jj] + first_nonzero - int(1.1 * dis_to_next_up)
                    )  # +int(dis_to_next_up*1./4.0)
                    point_down = (
                        peaks[jj] + first_nonzero + int(1.1 * dis_to_next_down)
                    )  # -int(dis_to_next_down*1./4.0)
                else:
                    point_up = (
                        peaks[jj] + first_nonzero - int(1.23 * dis_to_next_up)
                    )  # +int(dis_to_next_up*1./4.0)
                    point_down = (
                        peaks[jj] + first_nonzero + int(1.33 * dis_to_next_down)
                    )  # -int(dis_to_next_down*1./4.0)
            ...
            distances = [
                cv2.pointPolygonTest(
                    contour_text_interest_copy,
                    tuple(
                        int(x) for x in np.array([xv[mj], peaks[jj] + first_nonzero])
                    ),
                    True,
                )
            ...
            textline_boxes_rot.append(
                np.array(
                    [
                        [int(x_min_rot1), int(point_up_rot1)],
            ...
            textline_boxes.append(
                np.array(
                    [
                        [int(x_min), int(point_up)],
        ...
        distances = [
            cv2.pointPolygonTest(
                contour_text_interest_copy,
                tuple(int(x) for x in np.array([xv[mj], peaks[0] + first_nonzero])),
                True,
            )
            for mj in range(len(xv))
        ]
        ...
        for jj in range(len(peaks)):
            if jj == 0:
                ...
                if point_up < 0:
                    point_up = 1
                point_down = (
                    peaks_neg[1] + first_nonzero
                )  # peaks[jj] + first_nonzero + int(1. / 1.8 * dis_to_next)
            elif jj == 1:
                point_down = (
                    peaks_neg[1] + first_nonzero
                )  # peaks[jj] + first_nonzero + int(1. / 1.8 * dis_to_next)
        ...
        for jj in range(len(peaks)):
            ...
            if jj == 0:
                ...
                point_down = (
                    peaks[jj] + first_nonzero + int(1.0 / 1.9 * dis_to_next_down)
                )
...
def separate_lines_vertical(img_patch, contour_text_interest, thetha):
    ...
    (
        ...
        peaks_neg,
        rotation_matrix,
    ) = dedup_separate_lines(img_patch, contour_text_interest, thetha, 0)
...
def separate_lines_vertical_cont(
    img_patch, contour_text_interest, thetha, box_ind, add_boxes_coor_into_textlines
):
    kernel = np.ones((5, 5), np.uint8)
    pixel = 255
    min_area = 0
    ...
    contours_imgs, hierarchy = cv2.findContours(
    ...
    for i in range(len(contours_imgs)):
        ...
        img_contour = cv2.fillPoly(
            img_contour, pts=[contours_imgs[i]], color=(255, 255, 255)
        )
        ...
        contours_text_rot, _ = cv2.findContours(
            threshrot.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE
        )
...
def textline_contours_postprocessing(
    textline_mask,
    slope,
    contour_text_interest,
    box_ind,
    ...
    try:
        ...
        textline_mask_help = np.zeros(
            (
                textline_mask.shape[0] + int(2 * y_help),
                textline_mask.shape[1] + int(2 * x_help),
                3,
            )
        )
        textline_mask_help[
            y_help: y_help + textline_mask.shape[0],
            x_help: x_help + textline_mask.shape[1],
            :,
        ] = np.copy(textline_mask[:, :, :])
        ...
        img_contour = cv2.fillPoly(
            img_contour, pts=[contour_text_copy], color=(255, 255, 255)
        )
        ...
        img_contour_help = np.zeros(
            (
                img_contour.shape[0] + int(2 * y_help),
        ...
        img_contour_help[
            y_help: y_help + img_contour.shape[0],
            x_help: x_help + img_contour.shape[1],
            :,
        ] = np.copy(img_contour[:, :, :])
        ...
        len_con_text_rot = [
            len(contours_text_rot[ib]) for ib in range(len(contours_text_rot))
        ]
...
def separate_lines_new2(img_path, thetha, num_col, slope_region, plotter=None):
    ...
    img_patch_ineterst = img_path[
        :, :
    ]  # [peaks_neg_true[14]-dis_up:peaks_neg_true[15]+dis_down ,:]
    ...
    img_patch_ineterst = img_path[
        :, :
    ]  # [peaks_neg_true[14]-dis_up:peaks_neg_true[14]+dis_down ,:]
    ...
    for i in range(nxf):
        ...
        img_patch_separated_returned = rotate_image(
            img_patch_separated, -slopes_tile_wise[i]
        )
        ...
        img_patch_separated_returned_true_size = img_patch_separated_returned_true_size[
            :, margin: length_x - margin
        ]
        img_patch_ineterst_revised[:, index_x_d + margin: index_x_u - margin] = (
            img_patch_separated_returned_true_size
        )
...
def return_deskew_slop(img_patch_org, sigma_des, main_page=False, plotter=None):
    ...
    onset_x = int((img_resized.shape[1] - img_int.shape[1]) / 2.0)
    onset_y = int((img_resized.shape[0] - img_int.shape[0]) / 2.0)
    ...
    img_resized[
        onset_y: onset_y + img_int.shape[0], onset_x: onset_x + img_int.shape[1]
    ] = img_int[:, :]
    ...
    if main_page and img_patch_org.shape[1] > img_patch_org.shape[0]:
        ...
        try:
            ...
            ang_int = angels[
                np.argmax(var_res)
            ]  # angels_sorted[arg_final]#angels[arg_sort_early[arg_sort[arg_final]]]#angels[arg_fin]
        except:
            ang_int = 0
