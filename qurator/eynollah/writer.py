...
class EynollahXmlWriter:
    ...
    def __init__(
        self, *, dir_out, image_filename, curved_line, textline_light, pcgts=None
    ):
    ...
    def serialize_lines_in_marginal(
        self,
        marginal_region,
        ...
        for j in range(len(all_found_textline_polygons_marginals[marginal_idx])):
            ...
            for l in range(len(all_found_textline_polygons_marginals[marginal_idx][j])):
                if not (self.curved_line or self.textline_light):
                    if (
                        len(all_found_textline_polygons_marginals[marginal_idx][j][l])
                        == 2
                    ):
                        textline_x_coord = max(
                            0,
                            int(
                                (
                                    all_found_textline_polygons_marginals[marginal_idx][
                                        j
                                    ][l][0]
                                    + all_box_coord_marginals[marginal_idx][2]
                                    + page_coord[2]
                                )
                                / self.scale_x
                            ),
                        )
                        textline_y_coord = max(
                            0,
                            int(
                                (
                                    all_found_textline_polygons_marginals[marginal_idx][
                                    ...
                                    + all_box_coord_marginals[marginal_idx][0]
                                    + page_coord[0]
                                )
                                / self.scale_y
                            ),
                ...
                if (self.curved_line or self.textline_light) and np.abs(
                    slopes_marginals[marginal_idx]
                ) <= 45:
                    if (
                        len(all_found_textline_polygons_marginals[marginal_idx][j][l])
                        == 2
                ...
                elif (self.curved_line or self.textline_light) and np.abs(
                    slopes_marginals[marginal_idx]
                ) > 45:
                    if (
                        len(all_found_textline_polygons_marginals[marginal_idx][j][l])
    ...
    def serialize_lines_in_region(
        ...
        for j in range(len(all_found_textline_polygons[region_idx])):
            ...
            points_co = ""
            for idx_contour_textline, contour_textline in enumerate(
                all_found_textline_polygons[region_idx][j]
            ):
                if not (self.curved_line or self.textline_light):
                    if len(contour_textline) == 2:
                        textline_x_coord = max(
                            0,
                            int(
                                (contour_textline[0] + region_bboxes[2] + page_coord[2])
                                / self.scale_x
                            ),
                        )
                        textline_y_coord = max(
                            0,
                            int(
                                (contour_textline[1] + region_bboxes[0] + page_coord[0])
                                / self.scale_y
                            ),
                        )
                    else:
                ...
                if (self.curved_line or self.textline_light) and np.abs(
                    ...
                    if len(contour_textline) == 2:
                        points_co += str(
                            int((contour_textline[0] + page_coord[2]) / self.scale_x)
                        )
                        points_co += ","
                        points_co += str(
                            int((contour_textline[1] + page_coord[0]) / self.scale_y)
                        )
                    else:
                        points_co += str(
                            int((contour_textline[0][0] + page_coord[2]) / self.scale_x)
                        )
                        points_co += ","
                        points_co += str(
                            int((contour_textline[0][1] + page_coord[0]) / self.scale_y)
                        )
                elif (self.curved_line or self.textline_light) and np.abs(
                    slopes[region_idx]
                ) > 45:
    ...
    def build_pagexml_no_full_layout(
        ...
        pcgts = (
            self.pcgts
            if self.pcgts
            else create_page_xml(self.image_filename, self.height_org, self.width_org)
        )
        page = pcgts.get_Page()
        page.set_Border(
            BorderType(Coords=CoordsType(points=self.calculate_page_coords(cont_page)))
        )
        ...
        for mm in range(len(found_polygons_text_region)):
            textregion = TextRegionType(
                ...
                type_="paragraph",
                Coords=CoordsType(
                    points=self.calculate_polygon_coords(
                        found_polygons_text_region[mm], page_coord
                    )
                ),
            )
            page.add_TextRegion(textregion)
            self.serialize_lines_in_region(
                textregion,
                all_found_textline_polygons,
                mm,
                page_coord,
        ...
        for mm in range(len(found_polygons_text_region_img)):
            ...
            for lmm in range(len(found_polygons_text_region_img[mm])):
                points_co += str(
                    int(
                        (found_polygons_text_region_img[mm][lmm, 0, 0] + page_coord[2])
                        / self.scale_x
                    )
                )
                points_co += ","
                points_co += str(
                    int(
                        (found_polygons_text_region_img[mm][lmm, 0, 1] + page_coord[0])
                        / self.scale_y
                    )
                )
                points_co += " "
        ...
        for mm in range(len(polygons_lines_to_be_written_in_xml)):
            sep_hor = SeparatorRegionType(
                id=counter.next_region_id, Coords=CoordsType()
            )
            ...
            for lmm in range(len(polygons_lines_to_be_written_in_xml[mm])):
                points_co += str(
                    int(
                        (polygons_lines_to_be_written_in_xml[mm][lmm, 0, 0])
                        / self.scale_x
                    )
                )
                points_co += ","
                points_co += str(
                    int(
                        (polygons_lines_to_be_written_in_xml[mm][lmm, 0, 1])
                        / self.scale_y
                    )
                )
                points_co += " "
        ...
        for mm in range(len(found_polygons_tables)):
            ...
            for lmm in range(len(found_polygons_tables[mm])):
                points_co += str(
                    int(
                        (found_polygons_tables[mm][lmm, 0, 0] + page_coord[2])
                        / self.scale_x
                    )
                )
                points_co += ","
                points_co += str(
                    int(
                        (found_polygons_tables[mm][lmm, 0, 1] + page_coord[0])
                        / self.scale_y
                    )
                )
                points_co += " "
    ...
    def build_pagexml_full_layout(
        ...
        self.logger.debug(
            "len(found_polygons_text_region_h) %s", len(found_polygons_text_region_h)
        )
        ...
        for mm in range(len(polygons_lines_to_be_written_in_xml)):
            page.add_SeparatorRegion(
                ImageRegionType(
                    ...
                    Coords=CoordsType(
                        points=self.calculate_polygon_coords(
                            polygons_lines_to_be_written_in_xml[mm], [0, 0, 0, 0]
                        )
                    ),
                )
            )
